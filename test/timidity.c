...
static const struct option longopts[] = {
    ...
    {"soundfont", required_argument, NULL, TIM_OPT_SOUNDFONT},
...
__attribute__((noreturn)) static inline int parse_opt_h(const char *);
...
__attribute__((noreturn)) static inline int parse_opt_v(const char *);
...
static inline int parse_opt_soundfont(const char *);
__attribute__((noreturn)) static inline int parse_opt_fail(const char *);
            ...
            if ((tmp[i] == '"' || tmp[i] == '\'') && (terminator = strchr(tmp + i + 1, tmp[i])) != NULL)
            {
                if (!isspace(terminator[1]) && terminator[1] != '\0')
                {
                    ctl->cmsg(CMSG_ERROR, VERB_NORMAL,
                              ...
                              "string terminator (%c) and the next parameter",
                              name, line, tmp[i]);
                    CHECKERRLIMIT;
                    param_parse_err = 1;
                    break;
...
MAIN_INTERFACE int set_tim_opt_long(int c, char *optarg, int index)
{
    ...
    case TIM_OPT_SOUNDFONT:
        return parse_opt_soundfont(arg);
    default:
...
static inline int parse_opt_G1(const char *arg)
{
    ...
    for (sp = time_segments; sp != NULL; sp = sp->next)
    {
        if (sp->type != 1)
            continue;
        if (sp->begin.m.meas * 16 + sp->begin.m.beat <= prev_end_meas * 16 + prev_end_beat)
        {
            ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Segments must be ordered");
            free_time_segments();
            return 1;
        }
        else if (sp->end.m.meas != -1 && sp->end.m.beat != -1 && sp->begin.m.meas * 16 + sp->begin.m.beat >= sp->end.m.meas * 16 + sp->end.m.beat)
        {
            ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Segment time must be ordered");
            free_time_segments();
            return 1;
...
parse_opt_h(const char *arg)
{
    ...
    fp = open_pager();
    strcpy(version, (strcmp(midi_version, "current")) ? "version " : "");
    strcat(version, midi_version);
    help_args[0] = version;
...
#define tim_str(x) "x"
    ...
    if (!strcmp(tim_str(DEFAULT_RESAMPLATION), "resample_gauss"))
        fputs(" (default)", fp);
    fputs(NLS
          "                 -EFresamp affects the behavior of -N option" NLS,
          fp);
    fputs(NLS, fp);
...
static inline int parse_opt_O(const char *arg)
{
    ...
    while (*(++arg))
        ...
        case 'U': /* uLaw */
            ...
            pmp->encoding &= ~(PE_SIGNED | PE_16BIT | PE_24BIT | PE_ALAW | PE_BYTESWAP);
            break;
        case 'A': /* aLaw */
            pmp->encoding |= PE_ALAW;
            pmp->encoding &= ~(PE_SIGNED | PE_16BIT | PE_24BIT | PE_ULAW | PE_BYTESWAP);
            break;
        case 'x':
...
__attribute__((noreturn)) static inline int parse_opt_v(const char *arg)
{
...
#else
        "TiMidity++ ",
        (strcmp(midi_version, "current")) ? "version " : "",
        midi_version,
        NLS,
        NLS,
#endif
...
static inline int parse_opt_soundfont(const char *arg)
{
    int order, cutoff, isremove, reso, amp;
    order = cutoff = reso = amp = -1;
    char *sf_file = (char *)arg;
    add_soundfont(sf_file, order, cutoff, reso, amp);
    got_a_configuration = 1;
    return 0;
}

__attribute__((noreturn)) static inline int parse_opt_fail(const char *arg)
{
...
static inline FILE *open_pager(void)
{
#if defined(HAVE_POPEN) && defined(HAVE_ISATTY)
    char *pager;
...
int main(int argc, char **argv)
{
    ...
    nfiles = argc - optind;
    files = argv + optind;
    //if (nfiles > 0 && ctl->id_character != 'r' && ctl->id_character != 'A' && ctl->id_character != 'W' && ctl->id_character != 'N' && ctl->id_character != 'P')
