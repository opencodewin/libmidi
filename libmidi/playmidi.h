...
#define MIDI_EVENT_NOTE(ep) (ISDRUMCHANNEL((ep)->channel) ? (ep)->a : (((int)(ep)->a + note_key_offset + channel[ep->channel].key_shift) & 0x7f))
...
typedef struct
{
    ...
    int
        vibrato_phase,
        orig_vibrato_control_ratio, vibrato_control_ratio,
    ...
    int32
        modenv_volume,
        modenv_target, modenv_increment;
...
enum
{
    ...
    /* Special modules */
    MODULE_MIDI_SPECIAL1 = 0x70,
    MODULE_MIDI_DEBUG = 0x7f,
};
