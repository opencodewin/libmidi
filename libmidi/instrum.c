...
static int32 convert_vibrato_sweep(uint8 sweep, int32 vib_control_ratio)
{
    ...
    return (int32)(TIM_FSCALE((double)(vib_control_ratio)*SWEEP_TUNING, SWEEP_SHIFT) / (double)(play_mode->rate * sweep));
...
{
    return (rate) ? (int32)(0x200 * pow(2.0, rate / 17.0) * 44100 / play_mode->rate * control_ratio) << fast_decay : 0;
}
 ...
 */
static Instrument *load_gus_instrument(char *name, ToneBank *bank, int dr, int prog, char *infomsg)
{
    ...
    if (tone && tone->tunenum == 0 && tone->envratenum == 0 && tone->envofsnum == 0 && tone->tremnum == 0 && tone->vibnum == 0 && tone->sclnotenum == 0 && tone->scltunenum == 0 && tone->modenvratenum == 0 && tone->modenvofsnum == 0 && tone->envkeyfnum == 0 && tone->envvelfnum == 0 && tone->modenvkeyfnum == 0 && tone->modenvvelfnum == 0 && tone->trempitchnum == 0 && tone->tremfcnum == 0 && tone->modpitchnum == 0 && tone->modfcnum == 0 && tone->fcnum == 0 && tone->resonum == 0)
        if ((ip = search_instrument_cache(name, panning, amp, note_to_use, strip_loop, strip_envelope, strip_tail)) != NULL)
        {
    ...
    if (!(tf = open_file_r(name, 2, OF_NORMAL)))
    {
        ...
        for (i = 0; patch_ext[i]; i++)
        {
            ...
            {
                if (name_len >= ext_len && strcmp(name + name_len - ext_len, patch_ext[i]) == 0)
    ...
    if ((tf_read(tmp + 1, 1, 238, tf) != 238) || (memcmp(tmp, "GF1PATCH110\0ID#000002", 22) && memcmp(tmp, "GF1PATCH100\0ID#000002", 22)))
    {
    ...
    for (i = 0; i < ip->samples; i++)
    {
        ...
        READ_CHAR(tmp[0]);
        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Rate/Low/Hi/Root = %d/%d/%d/%d", sp->sample_rate, sp->low_freq, sp->high_freq, sp->root_freq);
        if (panning == -1)
        ...
        else
        {
            ...
            ctl->cmsg(CMSG_INFO, VERB_DEBUG, " * tremolo: sweep %d, phase %d, depth %d", sp->tremolo_sweep_increment, sp->tremolo_phase_increment, sp->tremolo_depth);
        }
        if (!tmp[16] || !tmp[17])
        {
        ...
        else
        {
            ...
            ctl->cmsg(CMSG_INFO, VERB_DEBUG, " * vibrato: sweep %d, ctl %d, depth %d", sp->vibrato_sweep_increment, sp->vibrato_control_ratio, sp->vibrato_depth);
        }
        READ_CHAR(sp->modes);
    ...
    close_file(tf);
    store_instrument_cache(ip, name, panning, amp, note_to_use, strip_loop, strip_envelope, strip_tail);
    return ip;
}
...
Instrument *load_instrument(int dr, int b, int prog)
{
    ...
    if (bank->tone[prog].instype == 1 || bank->tone[prog].instype == 2)
    {
        ...
        { /* Font extention */
            ...
            ip = extract_soundfont(bank->tone[prog].name, font_bank, font_preset, font_keynote);
        }
        ...
        if (ip != NULL && bank->tone[prog].amp != -1)
        {
            ...
            if (volume_max != 0)
                for (i = 0; i < ip->samples; i++)
                    ip->sample[i].volume *= bank->tone[prog].amp / 100.0 / volume_max;
        }
        /* panning */
    ...
    { /* load GUS/patch file */
        ...
        ip = load_gus_instrument(bank->tone[prog].name, bank, dr, prog, infomsg);
        if (ip == NULL)
