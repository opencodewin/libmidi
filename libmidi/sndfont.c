...
static void init_sf(SFInsts *rec)
{
    ...
    if (strcmp(rec->fname, "##memory") == 0)
    {
        ctl->cmsg(CMSG_INFO, VERB_NOISY, "Init internal soundfonts");
        URL url;
        if ((url = url_mem_open((char *)sfgm_bin, sfgm_size, 0)) == NULL)
        {
            ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Can't open internal soundfont");
            end_soundfont(rec);
            return;
        }
        struct midi_file *tf = (struct midi_file *)safe_malloc(sizeof(struct midi_file));
        tf->url = url;
        tf->tmpname = NULL;
        rec->tf = tf;
    }
...
void init_load_soundfont(void)
{
    if (!sfrecs)
    {
        // not add soundfont, we using internal font
        SFInsts *sf;
        sf = (SFInsts *)safe_malloc(sizeof(SFInsts));
        memset(sf, 0, sizeof(SFInsts));
        init_mblock(&sf->pool);
        sf->fname = SFStrdup(sf, "##memory");
        sf->def_order = DEFAULT_SOUNDFONT_ORDER;
        sf->amptune = 1.0;
        sfrecs = sf;
        init_sf(sf);
...
static Instrument *try_load_soundfont(SFInsts *rec, int order, int bank, int preset, int keynote)
{
    ...
    if (rec->tf == NULL)
    {
        ...
        if (strcmp(rec->fname, "##memory") == 0)
        {
            ...
            rec->tf = (struct midi_file *)safe_malloc(sizeof(struct midi_file));
            rec->tf->url = url;
            rec->tf->tmpname = NULL;
        }
        else if ((rec->tf = open_file(rec->fname, 1, OF_VERBOSE)) == NULL)
        {
            ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Can't open soundfont file %s", FILENAME_REDUCED(rec->fname));
            end_soundfont(rec);
            return NULL;
...
static Instrument *load_from_file(SFInsts *rec, InstList *ip)
{
    ...
    for (i = 0, sp = ip->slist; i < ip->samples && sp; i++, sp = sp->next)
    {
        ...
        if (i > 0 && (!sample->note_to_use || (sample->modes & MODES_LOOPING)))
        {
            SampleList *sps;
            Sample *found, *s;
...
static int parse_layer(SFInfo *sf, int pridx, LayerTable *tbl, int level)
{
    ...
    /* Here, it makes the reference of the data.  The real data
...
static void set_sample_info(SFInfo *sf, SampleList *vp, LayerTable *tbl)
{
    ...
    vp->v.loop_end = (tbl->val[SF_endloopAddrsHi] << 15) + tbl->val[SF_endloopAddrs] + sp->endloop - vp->start;
    ...
    /* Sample rate */
    if (sp->samplerate > 50000)
    {
        sp->samplerate = 50000;
    }
    else if (sp->samplerate < 400)
    {
        sp->samplerate = 400;
    }
...
static void set_init_info(SFInfo *sf, SampleList *vp, LayerTable *tbl)
{
    ...
    if (sample->sampletype == SF_SAMPLETYPE_MONO || val != 0)
    { /* monoSample = 1 */
        if (val < -500)
            vp->v.panning = 0;
        else if (val > 500)
...
static void set_rootkey(SFInfo *sf, SampleList *vp, LayerTable *tbl)
{
    ...
    if (tbl->set[SF_rootKey])
        ...
        vp->tune += (vp->keynote - sp->originalPitch) * 100 * (double)vp->v.scale_factor / 1024;
    ...
    temp = vp->v.modenv_to_pitch * (double)(1000 - tbl->val[SF_sustainEnv1]) / 1000 + 0.5;
...
static void set_rootfreq(SampleList *vp)
{
    ...
    if (root < 0)
    {
        vp->v.root_freq = freq_table[0] * (double)bend_fine[tune] / bend_coarse[-root] + 0.5;
        vp->v.scale_freq = 0; /* scale freq */
    }
    else if (root > 127)
    {
        vp->v.root_freq = freq_table[127] * (double)bend_fine[tune] * bend_coarse[root - 127] + 0.5;
...
static void convert_tremolo(SampleList *vp, LayerTable *tbl)
{
    ...
    {
        vp->v.tremolo_depth = -vp->v.tremolo_depth;
    }
...
PlayMode dpm = {
    DEFAULT_RATE, PE_16BIT | PE_SIGNED, PF_PCM_STREAM, -1, {0, 0, 0, 0, 0}, "null", 'n', NULL, NULL, NULL, NULL, NULL};
                    ...
                    else
                    {
                        ...
                        if (IS_SET_PROG_MAP(fft_range, x_preset, x_keynote))
                        {
                            ...
                            if (inst != NULL)
                            {
                                ...
                                if (freq != 260)
                                { /* 260 Hz is only returned when pitch is uncertain */
