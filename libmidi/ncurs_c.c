...
static void ctl_ext_help_mode(void);
...
enum ctl_ncurs_mode_t
{
    ...
    NCURS_MODE_EXT_HELP, /* Ext Help mode */
...
static void N_ctl_scrinit(void)
{
    ...
    waddstr(dftwin, "libmidi ");
    if (strcmp(midi_version, "current"))
        waddch(dftwin, 'v');
    waddstr(dftwin, midi_version);
    wmove(dftwin, VERSION_LINE, COLS - 51);
    waddstr(dftwin, "2022 Dicky Qi");
    ...
    memset(comment_indicator_buffer = (char *)safe_malloc(indicator_width), 0, indicator_width);
    memset(current_indicator_message =  (char *)safe_malloc(indicator_width), 0, indicator_width);
        ...
        else
        {
            if (IS_CURRENT_MOD_FILE)
            {
                wprintw(dftwin, "MOD %d (%s)",
                        ...
                        ChannelStatus[ch].comm ? ChannelStatus[ch].comm : "Not installed");
            }
            else
            {
                prog = &bank->tone[pr];
...
{
    if (ctl_cmdmode || ctl_ncurs_mode == NCURS_MODE_HELP || ctl_ncurs_mode == NCURS_MODE_EXT_HELP)
    {
        if (!ctl.trace_playing)
        {
        ...
        static char *help_message_list[] =
            {
                ...
                "h/?=Help mode  s=Toggle pause        E=ExtMode-Setting     e=ExtMode-Help",
...
{
    static WINDOW *helpwin;
    if (ctl_ncurs_mode == NCURS_MODE_EXT_HELP)
    {
        ctl_ncurs_mode = ctl_ncurs_back;
        touchwin(dftwin);
        delwin(helpwin);
    ...
    else
    {
        ...
        static char *help_message_list[] =
            {
                "Effect cmd:",
                "  Fdelay=d   Disable delay effect (default)",
                "  Fdelay=l   Enable Left delay",
                "    [,msec]      `msec' is optional to specify left-right delay time",
                "  Fdelay=r   Enable Right delay",
                "    [,msec]      `msec' is optional to specify left-right delay time",
                "  Fdelay=b   Enable rotate Both left and right",
                "    [,msec]      `msec' is optional to specify left-right delay time",
                "  Fchorus=d  Disable MIDI chorus effect control",
                "  Fchorus=n  Enable Normal MIDI chorus effect control",
                "    [,level]     `level' is optional to specify chorus level [0..127](default)",
                "  Fchorus=s  Surround sound, chorus detuned to a lesser degree",
                "    [,level]     `level' is optional to specify chorus level [0..127]",
                "  Freverb=d  Disable MIDI reverb effect control",
                "  Freverb=n  Enable Normal MIDI reverb effect control",
                "    [,level]     `level' is optional to specify reverb level [1..127]",
                "  Freverb=g  Global reverb effect",
                "    [,level]     `level' is optional to specify reverb level [1..127]",
                "  Freverb=f  Enable Freeverb MIDI reverb effect control (default)",
                "    [,level]     `level' is optional to specify reverb level [1..127]",
                "  Freverb=G  Global Freeverb effect",
                "    [,level]     `level' is optional to specify reverb level [1..127]",
                "  Fvlpf=d    Disable voice LPF",
                "  Fvlpf=c    Enable Chamberlin resonant LPF (12dB/oct) (default)",
                "  Fvlpf=m    Enable Moog resonant lowpass VCF (24dB/oct)",
                "  Fns=n      Enable the n th degree (type) noise shaping filter",
                "                 n:[0..4] (for 8-bit linear encoding, default is 4)",
                "                 n:[0..4] (for 16-bit linear encoding, default is 4)",
                "  Fresamp=d  Disable resamplation",
                "  Fresamp=l  Enable Linear resample algorithm",
                "  Fresamp=c  Enable C-spline resample algorithm",
                "  Fresamp=L  Enable Lagrange resample algorithm",
                "  Fresamp=n  Enable Newton resample algorithm",
                "  Fresamp=g  Enable Gauss-like resample algorithm",
                "             Fresamp affects the behavior of N option",
                NULL};
        ctl_ncurs_back = ctl_ncurs_mode;
        ctl_ncurs_mode = NCURS_MODE_EXT_HELP;
        helpwin = newwin(LIST_TITLE_LINES, COLS, TITLE_LINE, 0);
        N_ctl_werase(helpwin);
        wattron(helpwin, A_BOLD);
        waddstr(helpwin, "                 ncurses interface ext Help");
        wattroff(helpwin, A_BOLD);
...
static void ctl_list_mode(int type)
{
    ...
    {
        if (ctl_list_from[ctl_listmode_play] <= nc_playfile && nc_playfile <= ctl_list_to[ctl_listmode_play])
            break;
    }
    switch (type)
    {
...
static void ctl_event(CtlEvent *e)
{
    ...
    case CTLE_CURRENT_TIME:
        ctl_current_time((int)e->v1 / 1000, (int)e->v2);
        display_aq_ratio();
        break;
    case CTLE_NOTE:
...
static int ctl_cmd_L_enter(void)
{
    ...
    files[0] = text;
    nfiles = 1;
    new_files = files;//expand_file_archives(files, &nfiles);
    if (new_files == NULL)
    {
        rc = RC_NONE;
        beep();
...
static inline int parse_opt_default_mid(char *arg)
{
    ...
    if (!val)
    {
        wmove(dftwin, LINES - 1, 0);
        wattron(dftwin, A_REVERSE);
        waddstr(dftwin, "Manufacture ID: Illegal value");
        wattroff(dftwin, A_REVERSE);
        return 1;
    }
    opt_default_mid = val;
...
static inline int parse_opt_delay(const char *arg)
{
    ...
    if ((p = strchr(arg, ',')) != NULL)
        if ((effect_lr_delay_msec = atoi(++p)) < 0)
        {
            ...
            wmove(dftwin, LINES - 1, 0);
            wattron(dftwin, A_REVERSE);
            waddstr(dftwin, "Invalid delay parameter.");
            wattroff(dftwin, A_REVERSE);
            return 1;
        }
    return 0;
...
static inline int parse_opt_chorus(const char *arg)
{
    ...
    default:
        wmove(dftwin, LINES - 1, 0);
        wattron(dftwin, A_REVERSE);
        waddstr(dftwin, "Invalid chorus parameter.");
        wattroff(dftwin, A_REVERSE);
        return 1;
    }
    return 0;
                                  ...
                                  FLOAT_T i, FLOAT_T low, FLOAT_T high, const char *name, int f)
{
    if (i < low || i > high)
    {
        wmove(dftwin, LINES - 1, 0);
        wattron(dftwin, A_REVERSE);
        waddstr(dftwin, "Invalid float val.");
        wattroff(dftwin, A_REVERSE);
        return 1;
    }
    *param = i;
                             ...
                             const char *arg, FLOAT_T low, FLOAT_T high, const char *name, int f)
{
    ...
    if (arg == errp)
    {
        ...
        wmove(dftwin, LINES - 1, 0);
        wattron(dftwin, A_REVERSE);
        waddstr(dftwin, "Invalid float parameter.");
        wattroff(dftwin, A_REVERSE);
        return 1;
    }
...
static int parse_opt_reverb_freeverb(const char *arg, char type)
{
    ...
    if (*p && *p != ',')
    {
        if (set_value(&opt_reverb_control, atoi(p), 1, 0x7f, "Reverb level"))
            return 1;
        if (type == 'f')
    ...
    if (*p && *p != ',')
    {
        if (parse_val_float_t(&freeverb_scaleroom, p, 0, 10, "Freeverb scaleroom", 1))
            return 1;
    }
    if ((p = strchr(p, ',')) == NULL)
        return 0;
    ...
    if (*p && *p != ',')
    {
        if (parse_val_float_t(&freeverb_offsetroom, p, 0, 10, "Freeverb offsetroom", 1))
            return 1;
    }
    if ((p = strchr(p, ',')) == NULL)
        return 0;
    ...
    if (*p && *p != ',')
    {
        ...
        if (set_val_i32(&value, atoi(p), 0, 1000, "Freeverb predelay factor"))
            return 1;
...
static inline int parse_opt_reverb(const char *arg)
{
    ...
    default:
        wmove(dftwin, LINES - 1, 0);
        wattron(dftwin, A_REVERSE);
        waddstr(dftwin, "Invalid reverb parameter.");
        wattroff(dftwin, A_REVERSE);
        return 1;
    }
    return 0;
...
static inline int parse_opt_resample(const char *arg)
{
    ...
    default:
        wmove(dftwin, LINES - 1, 0);
        wattron(dftwin, A_REVERSE);
        waddstr(dftwin, "Invalid resample type.");
        wattroff(dftwin, A_REVERSE);
        return 1;
    }
    return 0;
    ...
    while (*arg)
    {
        ...
        case 'F':
            ...
            if (err)
            {
                wmove(dftwin, LINES - 1, 0);
                wattron(dftwin, A_REVERSE);
                waddstr(dftwin, "-E unsupported effect");
                wattroff(dftwin, A_REVERSE);
                return err;
            }
            return err;
...
static MFnode *make_new_MFnode_entry(char *file)
{
    ...
    cmsg(CMSG_WARNING, VERB_NORMAL, "%s: Not a midi file (Ignored)", url_unexpand_home_dir(file));
    return NULL;
}
...
static void display_aq_ratio(void)
{
    ...
    if ((devsiz = aq_get_dev_queuesize()) <= 0)
        return;
    rate = (int)(((double)(aq_filled() + aq_soft_filled()) / devsiz) * 100 + 0.5);
    //if (rate > 9999)
    //    rate = 10000;
    ...
    if (last_rate != rate)
    {
        ...
        //else
            wprintw(dftwin, " Audio queue: %5d%% ", rate);
        scr_modified_flag = 1;
    }
}
    ...
    if (indicator_msgptr == NULL)
    {
            ...
            current_indicator_chan = (current_indicator_chan + 1) % MAX_CHANNELS)
        {
            ...
            if (ChannelStatus[current_indicator_chan].prog != prog &&
                (ChannelStatus[current_indicator_chan].last_note_on + CHECK_NOTE_SLEEP_TIME > t))
                break;
        }
