...
static int get_importers(const char *sample_file, int limit, SampleImporter **importers)
{
    ...
    if (extension != NULL && (extension = strrchr(extension, '.')) != NULL)
    {
        ...
        while (importer->load != NULL && count < limit)
        {
            if (!importer->added && importer->extension != NULL && importer->discriminant != NULL && strcasecmp(extension, importer->extension) == 0)
            {
                ADD_IMPORTER
            }
            importer++;
...
#define READ_CHAR(thing)                  \
    if (1 != tf_read(&tmpchar, 1, 1, tf)) \
        goto fail;                        \
    thing = tmpchar;

#define READ_SHORT_LE(thing)               \
    if (1 != tf_read(&tmpshort, 2, 1, tf)) \
        goto fail;                         \
    thing = LE_SHORT(tmpshort);
#define READ_LONG_LE(thing)               \
    if (1 != tf_read(&tmplong, 4, 1, tf)) \
        goto fail;                        \
...
static int read_sample_data(int32 flags, struct midi_file *tf, int bits, int samples, int frames, sample_t **sdata);
...
static int read_WAVFormatChunk(struct midi_file *tf, WAVFormatChunk *fmt, int psize);
static int read_WAVSamplerChunk(struct midi_file *tf, WAVSamplerChunk *smpl, int psize);
static int read_WAVInstrumentChunk(struct midi_file *tf, GeneralInstrumentInfo *inst, int psize);
...
static int import_wave_discriminant(char *sample_file)
{
    ...
    if ((tf = open_file(sample_file, 1, OF_NORMAL)) == NULL)
        return 1;
    if (tf_read(buf, 12, 1, tf) != 1 || memcmp(&buf[0], "RIFF", 4) != 0 || memcmp(&buf[8], "WAVE", 4) != 0)
    {
        close_file(tf);
        return 1;
    }
...
static int import_wave_load(char *sample_file, Instrument *inst)
{
    ...
    for (;;)
    {
        ...
        if (memcmp(&buf[4 + 0], "fmt ", 4) == 0)
        {
            ...
            if (format.wChannels < 1                                                /* invalid range */
                || format.wChannels > MAX_SAMPLE_CHANNELS || format.wFormatTag != 1 /* compressed */
        ...
        else if (memcmp(&buf[4 + 0], "data", 4) == 0)
        {
            ...
            ctl->cmsg(CMSG_INFO, VERB_NOISY, "Format: %d-bits %dHz %dch, %d frames", format.wBitsPerSample, format.dwSamplesPerSec, samples, frames);
    ...
    if (chunk_flags & WAVE_CHUNKFLAG_SAMPLER)
    {
        ...
        if (samplerc.dwMIDIPitchFraction != 0 && samplerc.dwMIDIUnityNote != 127) /* no table data */
        {
            int32 diff;
...
static int read_WAVFormatChunk(struct midi_file *tf, WAVFormatChunk *fmt, int csize)
{
    int32 tmplong;
    int16 tmpshort;
...
static int read_WAVSamplerChunk(struct midi_file *tf, WAVSamplerChunk *smpl, int psize)
{
    ...
    if (tf_seek(tf, psize, SEEK_CUR) == -1)
        goto fail;
    ctl->cmsg(CMSG_INFO, VERB_NOISY, "Sampler: %dns/frame, note=%d, loops=%d", smpl->dwSamplePeriod, smpl->dwMIDIUnityNote, loopCount);
    return 1;
fail:
...
static int read_AIFFCommonChunk(struct midi_file *tf, AIFFCommonChunk *comm, int csize, int compressed);
static int read_AIFFSoundDataChunk(struct midi_file *tf, AIFFSoundDataChunk *sound, int csize, int mode);
static int read_AIFFSoundData(struct midi_file *tf, Instrument *inst, AIFFCommonChunk *common);
static int read_AIFFInstumentChunk(struct midi_file *tf, GeneralInstrumentInfo *inst, AIFFLoopInfo *loop, int csize);
static int read_AIFFMarkerChunk(struct midi_file *tf, AIFFMarkerData **markers, int csize);
...
static int import_aiff_discriminant(char *sample_file)
{
    ...
    if ((tf = open_file(sample_file, 1, OF_NORMAL)) == NULL)
        return 1;
    if (tf_read(buf, 12, 1, tf) != 1 || memcmp(&buf[0], "FORM", 4) != 0 || memcmp(&buf[8], "AIF", 3) != 0 || (buf[8 + 3] != 'F' && buf[8 + 3] != 'C'))
    {
        close_file(tf);
        return 1;
    }
...
static int import_aiff_load(char *sample_file, Instrument *inst)
{
    ...
    if (chunk_flags & AIFF_CHUNKFLAG_FAILED || (chunk_flags & AIFF_CHUNKFLAG_REQUIRED) != AIFF_CHUNKFLAG_REQUIRED)
    {
        if (marker_data != NULL)
            free(marker_data);
        close_file(tf);
    ...
    if (chunk_flags & AIFF_CHUNKFLAG_INSTRUMENT)
    {
        ...
        if ((loop_info.mode == 1 || loop_info.mode == 2) && chunk_flags & AIFF_CHUNKFLAG_MARKER && marker_data != NULL)
        {
            Sample *sample;
            int i;
            ...
            if (AIFFGetMarkerPosition(loop_info.beginID, marker_data, &loopStart) && AIFFGetMarkerPosition(loop_info.endID, marker_data, &loopEnd))
            {
...
static int read_AIFFCommonChunk(struct midi_file *tf, AIFFCommonChunk *comm, int csize, int compressed)
{
    ...
    csize -= 8 + 10;
    ctl->cmsg(CMSG_INFO, VERB_NOISY, "Format: %d-bits %dHz %dch, %d frames", comm->sampleSize, (int)comm->sampleRate, comm->numChannels, comm->numSampleFrames);
    if (compressed)
    {
...
#define BLOCK_READ_BEGIN(stype, sbyte, fch)                                                                    \
    { /* sbyte may be sizeof(stype) */                                                                         \
...
static int read_sample_data(int32 flags, struct midi_file *tf, int bits, int channels, int frames, sample_t **sdata)
{
    ...
    else
    {
        if (channels == 1)
        {
            if (flags & SAMPLE_8BIT_UNSIGNED)
            {
                ...
                {
                    sdata[0][i] = BITS_U8_TO_16(data[j]);
                    j++;
                }
                BLOCK_READ_END
            }
            else
            {
                BLOCK_READ_BEGIN(uint8, 1, 1)
                {
                    sdata[0][i] = BITS_S8_TO_16(data[j]);
                    j++;
                }
                BLOCK_READ_END
            }
