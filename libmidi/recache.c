...
#define RESAMPLATION_CACHE                  \
    _x = do_resamplation(src, ofs, &resrc); \
...
struct cache_hash *resamp_cache_fetch(Sample *sp, int note)
{
    ...
    if (sp->vibrato_control_ratio || (sp->modes & MODES_PINGPONG) || (sp->sample_rate == play_mode->rate && sp->root_freq == get_note_freq(sp, sp->note_to_use)))
        return NULL;
...
void resamp_cache_refer_on(Voice *vp, int32 sample_start)
{
    ...
    ch = vp->channel;
    if (vp->vibrato_control_ratio || channel[ch].portamento || (vp->sample->modes & MODES_PINGPONG) || vp->orig_frequency != vp->frequency || (vp->sample->sample_rate == play_mode->rate && vp->sample->root_freq == get_note_freq(vp->sample, vp->sample->note_to_use)))
        return;
    note = vp->note;
...
void resamp_cache_refer_off(int ch, int note, int32 sample_end)
{
    ...
    if (p == NULL)
        return;
    sp = p->sp;
    if (sp->sample_rate == play_mode->rate && sp->root_freq == get_note_freq(sp, sp->note_to_use))
        return;
    ...
    if (!(sp->modes & MODES_LOOPING))
    {
        ...
        a = ((double)sp->root_freq * play_mode->rate) / ((double)sp->sample_rate * get_note_freq(sp, note));
