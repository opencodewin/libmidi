 ...
 *
 * -V 1.661, or (1/log10(4)), is the default volume curve for GM/XG,
 * and is thus the default for MOD->MIDI output as well, so that the
 * resulting midi will sound correct when played using default options.
 *
...
void m2m_process_events(MidiEvent *ev)
{
    ...
    for (; ev->type != ME_EOT; ev++)
    {
        /* convert times to midi event times */
