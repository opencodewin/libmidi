...
{
    int32 nsamples = (play_mode->encoding & PE_MONO) ? count : count * 2;
    ...
    if (opt_reverb_control == 2 || opt_reverb_control == 4 || (opt_reverb_control < 0 && !(opt_reverb_control & 0x80)) || opt_chorus_control < 0)
    {
...
#endif
        if (opt_reverb_control == 2 || opt_reverb_control == 4 || (opt_reverb_control < 0 && !(opt_reverb_control & 0x80)))
...
static void ns_shaping8(int32 *lp, int32 c)
{
    ...
    for (i = 0; i < c; i++)
    {
        ...
        if (lp[i] > NS_AMP_MAX)
            ...
            lp[i] = NS_AMP_MIN;
        ll = lp[i] + ns_tap_0 * ns_z0[0] + ns_tap_1 * ns_z0[1] + ns_tap_2 * ns_z0[2] + ns_tap_3 * ns_z0[3];
        ...
        if (lp[i] > NS_AMP_MAX)
            ...
            lp[i] = NS_AMP_MIN;
        ll = lp[i] + ns_tap_0 * ns_z1[0] + ns_tap_1 * ns_z1[1] + ns_tap_2 * ns_z1[2] + ns_tap_3 * ns_z1[3];
        l = ll >> (32 - 8 - GUARD_BITS);
        lp[i] = l << (32 - 8 - GUARD_BITS);
...
static void do_soft_clipping1(int32 *buf, int32 count)
{
    ...
    {
        x = buf[i];
        x = (x > WS_AMP_MAX) ? WS_AMP_MAX : ((x < WS_AMP_MIN) ? WS_AMP_MIN : x);
...
static void ns_shaping16_9(int32 *lp, int32 c)
{
    ...
    for (i = 0; i < c; i++)
    {
        ...
        lp[i] = (lp[i] > NS_AMP_MAX)   ? NS_AMP_MAX
                : (lp[i] < NS_AMP_MIN) ? NS_AMP_MIN
                : lp[i];
        sample = lp[i] - ns9_coef[8] * ns9_ehl[ns9_histposl + 8] - ns9_coef[7] * ns9_ehl[ns9_histposl + 7] - ns9_coef[6] * ns9_ehl[ns9_histposl + 6] - ns9_coef[5] * ns9_ehl[ns9_histposl + 5] - ns9_coef[4] * ns9_ehl[ns9_histposl + 4] - ns9_coef[3] * ns9_ehl[ns9_histposl + 3] - ns9_coef[2] * ns9_ehl[ns9_histposl + 2] - ns9_coef[1] * ns9_ehl[ns9_histposl + 1] - ns9_coef[0] * ns9_ehl[ns9_histposl];
        ...
        lp[i] = (lp[i] > NS_AMP_MAX)   ? NS_AMP_MAX
                : (lp[i] < NS_AMP_MIN) ? NS_AMP_MIN
                : lp[i];
        sample = lp[i] - ns9_coef[8] * ns9_ehr[ns9_histposr + 8] - ns9_coef[7] * ns9_ehr[ns9_histposr + 7] - ns9_coef[6] * ns9_ehr[ns9_histposr + 6] - ns9_coef[5] * ns9_ehr[ns9_histposr + 5] - ns9_coef[4] * ns9_ehr[ns9_histposr + 4] - ns9_coef[3] * ns9_ehr[ns9_histposr + 3] - ns9_coef[2] * ns9_ehr[ns9_histposr + 2] - ns9_coef[1] * ns9_ehr[ns9_histposr + 1] - ns9_coef[0] * ns9_ehr[ns9_histposr];
        l = sample >> (32 - 16 - GUARD_BITS);
