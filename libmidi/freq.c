...
float freq_fourier(Sample *sp, int *chord)
{
    ...
    if (max_guessfreq >= (rate >> 1))
        max_guessfreq = (rate >> 1) - 1;
    ...
    if (maxpitch2 > HIGHEST_PITCH)
        maxpitch2 = HIGHEST_PITCH;
    ...
    /* assign chords */
    if ((pitch = assign_chord(new_pitchbins, chord, bestpitch - 9, maxpitch2, bestpitch)) >= 0)
        bestpitch = pitch;
