...
static int read_rcp_track(struct midi_file *tf, int trackno, int gfmt);
...
int read_rcp_file(struct midi_file *tf, char *magic0, char *fn)
{
    ...
    if (!gfmt) /* RCP or R36 */
    {
        ...
        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Time signature(n) %d", current_file_info->time_sig_n);
        current_file_info->time_sig_d = tf_getc(tf);
        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Time signature(d) %d", current_file_info->time_sig_d);
    ...
    else /* G36 or G18 */
    {
        ...
        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Time signature(n) %d", current_file_info->time_sig_n);
        current_file_info->time_sig_d = tf_getc(tf);
        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Time signature(n) %d", current_file_info->time_sig_d);
        init_keysig = tf_getc(tf);
        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Key signature %d", init_keysig);
        if (init_keysig < 0 || init_keysig >= 32)
            init_keysig = 0;
    ...
    for (i = 0; i < 8; i++)
    {
        int mid;
        skip(tf, 24); /* memo */
        if (tf_read(user_exclusive_data[i], 1, USER_EXCLUSIVE_LENGTH, tf) != USER_EXCLUSIVE_LENGTH)
            return 1;
    ...
    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "divisions %d", current_file_info->divisions);
    current_file_info->format = 1;
...
static char *rcp_cmd_name(int cmd)
{
    ...
    {
    case 0x90:
        return "UserExclusive0";
    case 0x91:
        return "UserExclusive1";
    case 0x92:
        return "UserExclusive2";
    case 0x93:
        return "UserExclusive3";
    case 0x94:
        return "UserExclusive4";
    case 0x95:
        return "UserExclusive5";
    case 0x96:
        return "UserExclusive6";
    case 0x97:
        return "UserExclusive7";
    case 0x98:
    ...
    case 0xc3:
        return "TX function";
    case 0xc5:
        return "FB-01 P parameter";
    case 0xc6:
        return "FB-01 S System";
    case 0xc7:
        return "TX81Z V VCED";
    case 0xc8:
        return "TX81Z A ACED";
    case 0xc9:
        return "TX81Z P PCED";
    case 0xca:
        return "TX81Z S System";
    case 0xcb:
        return "TX81Z E EFFECT";
    case 0xcc:
        return "DX7-2 R REMOTE SW";
    case 0xcd:
        return "DX7-2 A ACED";
    case 0xce:
        return "DX7-2 P PCED";
...
static int read_rcp_track(struct midi_file *tf, int trackno, int gfmt)
{
    ...
    if (ch >= RCP_MAXCHANNELS)
        ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "RCP: Invalid channel: %d", ch);
    ...
    {
        ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "RCP: Invalid time offset: %d", time_offset);
        if (time_offset < -99)
            time_offset = -99;
        else
            time_offset = 99;
    ...
    while (last_point >= tf_tell(tf) + cmdlen)
    {
        ...
        case 0x97: /* User Exclusive 8 */
            memcpy(sysex, user_exclusive_data[cmd - 0x90], USER_EXCLUSIVE_LENGTH);
        ...
        case 0xd1: /* Yamaha para */
            ...
            b &= 0x7f;
            if (!yamaha_base_init)
                ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "YamPara used before initializing YamBase");
            if (!yamaha_dev_init)
                ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "YamPara used before initializing YamDev#");
        ...
        case 0xde: /* Roland para */
            ...
            b &= 0x7f;
            if (!roland_base_init)
                ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "RolPara used before initializing RolBase");
            if (!roland_dev_init)
                ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "RolPara used before initializing RolDev#");
        ...
        case 0xe1: /* BnkLPrg */
            ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "BnkLPrg is not supported: 0x%02x 0x%02x", a, b);
            ntr_incr(&ntr, step);
            break;
...
#endif
            ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Key scan 0x%02x 0x%02x is not supported", a, b);
            ntr_incr(&ntr, step);
            break;
        ...
        case 0xe7: /* tempo change */
            if (a == 0)
            {
                ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Invalid tempo change\n");
                a = 64;
            }
        ...
        case 0xf6: /* comment */
            ...
            if (ctl->verbosity >= VERB_VERBOSE || opt_trace_text_meta_event)
            {
                ...
                {
                    MidiEvent ev;
                    if (readmidi_make_string_event(ME_TEXT, buff, &ev, 1) != NULL)
                    {
                        ev.time = ntr_at(ntr);
                        readmidi_add_event(&ev);
                    }
        ...
        case 0xf7: /* 2nd Event */
            ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Something's wrong: 2nd Event is appeared");
            break;

        case 0xf8: /* loop end */
            if (ctl->verbosity >= VERB_DEBUG_SILLY)
                ctl->cmsg(CMSG_INFO, VERB_DEBUG_SILLY,  "Loop end at %d", tf_tell(tf) - track_top - cmdlen);
            if (sp == 0)
                ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Misplaced loop end (ch=%d)", ch);
            ...
            else
            {
                ...
                {
                    ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Loop limit exceeded (ch=%d)", ch);
                    sp = top;
                }
#endif /* RCP_LOOP_CONT_LIMIT */
#ifdef RCP_LOOP_TIME_LIMIT
                else if ((current_tempo / 1000000.0) * (ntr_at(ntr) - stack[top].start_at) / current_file_info->divisions > RCP_LOOP_TIME_LIMIT)
                {
                    ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Loop time exceeded (ch=%d)", ch);
                    sp = top;
                }
...
#endif
            if (ctl->verbosity >= VERB_DEBUG_SILLY)
                ctl->cmsg(CMSG_INFO, VERB_DEBUG_SILLY, "Loop start at %d", tf_tell(tf) - track_top - cmdlen);

            if (sp >= MAX_STACK_DEPTH)
                ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Too deeply nested %d (ch=%d, Ignored this loop)", ch, sp);
            else
            {
        ...
        case 0xfc: /* same measure */
            if (ctl->verbosity >= VERB_DEBUG_SILLY)
                ctl->cmsg(CMSG_INFO, VERB_DEBUG_SILLY, "Same measure at %d", tf_tell(tf) - track_top - cmdlen);
            if (sp >= MAX_STACK_DEPTH)
                ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Too deeply nested %d (ch=%d)", sp, ch);
            else
            {
                long jmp, nextflag;
            ...
            next_same_measure:
                if (!gfmt)
                {
                    ...
                    if (jmp & 0x03) /* ##?? */
                    {
                        ...
                        /* Clear them here. */
                        ctl->cmsg(CMSG_WARNING, VERB_DEBUG, "Jump %d is changed to %d", jmp, jmp & ~3);
                        jmp &= ~3; /* jmp=(jmp/4)*4 */
                    }
                }
                else
                ...
                {
                    ctl->cmsg(CMSG_WARNING, VERB_NOISY, "RCP Invalid same measure: %d (ch=%d)", jmp, ch);
                    break;
                }
        ...
        default:
            if (!gfmt)
            {
                ...
                {
                    ctl->cmsg(CMSG_ERROR, VERB_VERBOSE, "RCP e9 f0 e0 80 is end of track ??");
                    goto end_of_track;
                }
            }
            else
...
static void ntr_add(struct RCPNoteTracer *ntr, int ch, int note, int gate)
{
    ...
    if (ntr->freelist)
    {
        ...
        p = (struct NoteList *)new_segment(&ntr->pool, sizeof(struct NoteList));
    p->gate = gate;
    p->ch = ch;
    p->note = note;
    p->next = ntr->notes;
...
static void ntr_incr(struct RCPNoteTracer *ntr, int step)
{
    ...
    while (step >= 0)
    {
        ...
        while (p)
        {
            ...
            {
                if (ctl->verbosity >= VERB_DEBUG_SILLY)
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG_SILLY, "NoteOff %d at %d", p->note, ntr->at);
...
static void ntr_wait_all_off(struct RCPNoteTracer *ntr)
{
    while (ntr->notes)
    {
        ...
        while (p)
        {
            ...
            {
                if (ctl->verbosity >= VERB_DEBUG_SILLY)
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG_SILLY, "NoteOff %d", p->note);
                MIDIEVENT(ntr->at, ME_NOTEOFF, p->ch, p->note, 0);
                p->next = ntr->freelist;
                ntr->freelist = p;
            }
