...
static int tf_read_beint16(int *, midi_file *);
static int tf_read_beint32(int *, midi_file *);
static int read_mfi_information(int, int *, int *, int *, midi_file *);
static int read_mfi_track(int, int, int, int, int, midi_file *);

int read_mfi_file(midi_file *tf)
{
    ...
    if (tf_read_beint32(&dataLength, tf) != 1 || tf_read_beint16(&infoLength, tf) != 1 || tf_read_beint16(&dataType, tf) != 1 || tf_read(&numTracks, 1, 1, tf) != 1)
        return 1;
    infoLength -= 2 + 1;
    if (dataType == 0x0202)
    {
    ...
    {
        if (tf_read(&type, 4, 1, tf) != 1 || tf_read_beint32(&length, tf) != 1)
            return 1;
...
static int read_mfi_information(int infoLength, int *mfiVersion, int *noteType, int *extStDLength, midi_file *tf)
{
    int length;
    uint32 type;
    uint8 byteData;
    ...
    *mfiVersion = 1;
    while (infoLength > 0)
    {
        ...
        infoLength -= 6;
        if (tf_read(&type, 4, 1, tf) != 1 || tf_read_beint16(&length, tf) != 1)
            return 1;
        if (length == 0)
            continue;
        ...
        case BE_FCC(0x76657273 /*vers*/): /* version (unused) */
            ...
            switch (type)
            {
            case BE_FCC(0x30313030 /*0100*/):
                *mfiVersion = 1;
                break;
            case BE_FCC(0x30323030 /*0200*/):
                *mfiVersion = 2;
                break;
            case BE_FCC(0x30333030 /*0300*/):
                *mfiVersion = 3;
                break;
            default:
...
char *get_mfi_file_title(midi_file *tf)
{
    int length, dataLength, infoLength, dataType;
    uint32 type;
    uint8 numTracks;

    if (tf_read_beint32(&dataLength, tf) != 1 || tf_read_beint16(&infoLength, tf) != 1 || tf_read_beint16(&dataType, tf) != 1 || dataType == 0x0202 || tf_read(&numTracks, 1, 1, tf) != 1)
        return NULL;
    infoLength -= 2 + 1;
...
#define SEND_LASTNOTEINFO(lni, ch)        \
    if (LASTNOTEINFO_HAS_DATA((lni)[ch])) \
        SendLastNoteInfo(lni, ch);
#define SEND_AND_CLEAR_LASTNOTEINFO(lni, ch) \
    if (LASTNOTEINFO_HAS_DATA((lni)[ch]))    \
    {                                        \
        SendLastNoteInfo(lni, ch);           \
        (lni)[ch].on = NO_LAST_NOTE_INFO;    \
    }
...
#define CHECK_AND_READ_FROM_FILE(ptr, readLen)                         \
    do                                                                 \
    {                                                                  \
...
static int read_mfi_track(int trackNo, int length, int mfiVersion, int noteType, int extStDLength, midi_file *tf)
{
    uint8 data[5];
    ...
    while (length > 0)
    {
        ...
        if (data[1] != 0xFF) /* note */
        {
            ...
            {
                if (lastNotes[channel].off <= pos || note != lastNotes[channel].note)
                {
...
#define GET_PART_AND_CHANNEL(p, c) \
    (p) = data[3] >> 6;            \
    (c) = channelMap[p]
...
static int tf_read_beint16(int *value, midi_file *tf)
{
    uint16 value_;
...
static int tf_read_beint32(int *value, midi_file *tf)
{
    uint32 value_;
