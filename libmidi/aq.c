...
void aq_setup(void)
{
    ...
    if (IS_STREAM_TRACE)
    {
        ...
        {
            ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Warning: Audio buffer is too small.");
            device_qsize = 0;
        }
        else
        {
            device_qsize -= device_qsize % Bps; /* Round Bps */
            ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Audio device queue size: %d bytes", device_qsize);
            ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Write bucket size: %d bytes (%d msec)", bucket_size, (int)(bucket_time * 1000 + 0.5));
        }
    }
    else
    {
...
retry:
    ...
    for (;;)
    {
        ...
        {
            ctl->cmsg(CMSG_WARNING, VERB_DEBUG, "Warning: Audio test is terminated");
            break;
        }
        play_mode->output_data(nullsound, bucket_size);
        diff = get_current_calender_time() - start;

        if (diff > chunktime || qbytes > 1024 * 512 || chunktime < diff)
            break;
    ...
    if (bucket_size * 2 > qbytes)
    {
        if (ntries == 4)
        {
            ctl->cmsg(CMSG_ERROR, VERB_NOISY, "Can't estimate audio queue length");
            set_bucket_size(audio_buffer_size * Bps);
            free(nullsound);
            return 2 * audio_buffer_size * Bps;
        }

        ctl->cmsg(CMSG_WARNING, VERB_DEBUG, "Retry to estimate audio queue length (%d times)", ntries);
...
int aq_add(int32 *samples, int32 count, int c)
{
    int32 nbytes, i;
    char *buff;
    ...
    if (c && ctl->flags & CTLF_OUTPUTRAWDATA && ctl->write)
    {
        ctl->write(buff, nbytes);
    }
...
int aq_flush(int discard)
{
    ...
    if (discard)
    {
        ...
        ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "ERROR: Can't discard audio buffer");
    }
