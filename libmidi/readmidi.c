...
/* rcp.c */
int read_rcp_file(struct midi_file *tf, char *magic0, char *fn);

/* mld.c */
extern int read_mfi_file(struct midi_file *tf);
extern char *get_mfi_file_title(struct midi_file *tf);
...
void readmidi_add_event(MidiEvent *a_event)
{
    ...
    if (event_count == MAX_MIDI_EVENT)
    {
        if (!readmidi_error_flag)
        {
            ...
            ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Maxmum number of events is exceeded");
        }
        return;
    }
    event_count++;
...
char *readmidi_make_string_event(int type, char *string, MidiEvent *ev, int cnv)
{
    char *text;
    int len;
    StringTableNode *st;
...
static int32 getvl(struct midi_file *tf)
{
    int32 l;
    int c;
    ...
    /* Error */
    ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "%s: Illigal Variable-length quantity format.", current_filename);
    return -2;
...
eof:
    if (errno)
        ...
        ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Warning: %s: Too shorten midi file.", current_filename);
    return -1;
}
              ...
              (val[1] >= 0x10 && val[3] == 0x08)))    /* Parameter Change */
    {
        ...
        for (ent = addlow; body <= body_end; body++, ent++)
        {
            ...
            case 0x05: /* mono/poly mode */
                if (*body == 0)
                {
                    SETMIDIEVENT(evm[num_events], 0, ME_MONO, p, 0, SYSEX_TAG);
                }
                else
                {
                    SETMIDIEVENT(evm[num_events], 0, ME_POLY, p, 0, SYSEX_TAG);
                }
                num_events++;
                break;
             ...
             val[3] == 0x12)   /* Data Set Command */
    {
        ...
        case 0x40:
            if ((addr & 0xFFF000) == 0x401000)
            {
                ...
                case 0x02: /* Rx. Channel */
                    if (val[7] == 0x10)
                    {
                        SETMIDIEVENT(evm[0], 0, ME_SYSEX_GS_LSB, block_to_part(val[5], midi_port_number ^ port), 0x80, 0x45);
                    }
                    else
                    {
                        SETMIDIEVENT(evm[0], 0, ME_SYSEX_GS_LSB, block_to_part(val[5], midi_port_number ^ port), MERGE_CHANNEL_PORT2(val[7], midi_port_number ^ port), 0x45);
                    }
                    num_events++;
                    break;
                ...
                case 0x13: /* MONO/POLY Mode */
                    if (val[7] == 0)
                    {
                        SETMIDIEVENT(evm[0], 0, ME_MONO, p, val[7], SYSEX_TAG);
                    }
                    else
                    {
                        SETMIDIEVENT(evm[0], 0, ME_POLY, p, val[7], SYSEX_TAG);
                    }
                    num_events++;
                    break;
                ...
                case 0x40: /* Scale Tuning */
                    for (i = 0; i < 12; i++)
                    {
                        ...
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "Scale Tuning %s (CH:%d %d cent)", note_name[i], p, val[i + 7] - 64);
                    }
                    num_events += 12;
                    break;
                default:
        ...
        case 0x21: /* User Drumset */
            ...
            case 0x300: /* Assign Group */
                ...
                if (val[7] != 0)
                {
                    recompute_userdrum_altassign(udn + 64, val[7]);
                }
                break;
            case 0x400: /* Panpot */
        ...
        case 0x00: /* System */
            ...
            case 0x0100: /* Channel Msg Rx Port (A) */
                SETMIDIEVENT(evm[0], 0, ME_SYSEX_GS_LSB, block_to_part(addr & 0xf, 0), val[7], 0x46);
                num_events++;
                break;
            case 0x0110: /* Channel Msg Rx Port (B) */
                SETMIDIEVENT(evm[0], 0, ME_SYSEX_GS_LSB, block_to_part(addr & 0xf, 1), val[7], 0x46);
                num_events++;
                break;
            default:
     ...
     */
        ...
        case 0x08: /* MIDI Tuning Standard */
            ...
            case 0x01:
                ...
                break;
            case 0x02:
                SETMIDIEVENT(evm[0], 0, ME_SINGLE_NOTE_TUNING, 0, val[4], 0);
                for (i = 0; i < val[5]; i++)
                {
            ...
            case 0x0b:
                ...
                if (val[1] == 0x7f)
                {
                    SETMIDIEVENT(evm[0], 0, ME_MASTER_TEMPER_TYPE, 0, val[7], (val[0] == 0x7f));
                    num_events++;
                }
                else
                {
                    for (i = j = 0; i < 32; i++)
                        if (channel_tt & 1 << i)
                        {
                            SETMIDIEVENT(evm[j], 0, ME_TEMPER_TYPE, MERGE_CHANNEL_PORT(i), val[7], (val[0] == 0x7f));
                            j++;
                        }
                    num_events += j;
                }
                break;
            case 0x0c:
                SETMIDIEVENT(evm[0], 0, ME_USER_TEMPER_ENTRY, 0, val[4], val[21]);
                for (i = 0; i < val[21]; i++)
                {
...
int parse_sysex_event(uint8 *val, int32 len, MidiEvent *ev)
{
        ...
        val[3] == 0x12)   /* Data Set Command */
    {
        ...
        if (addr == 0x400004) /* Master Volume */
        {
            ...
            SETMIDIEVENT(*ev, 0, ME_MASTER_VOLUME, 0, vol & 0xFF, (vol >> 8) & 0xFF);
            return 1;
        }
    ...
    if (len > 4 && val[0] >= 0x7e)
        ...
        case 0x04: /* MIDI Time Code Setup/Device Control */
            ...
            case 0x01: /* Master Volume */
                ...
                {
                    SETMIDIEVENT(*ev, 0, ME_MAINVOLUME, MERGE_CHANNEL_PORT(val[1]), vol >> 8 & 0xff, 0);
                }
                return 1;
            }
            break;
...
static void smf_time_signature(int32 at, struct midi_file *tf, int len)
{
    int n, d, c, b;
    ...
    ctl->cmsg(CMSG_INFO, VERB_NOISY, "Time signature: %d/%d %d clock %d q.n.", n, d, c, b);
...
static void smf_key_signature(int32 at, struct midi_file *tf, int len)
{
    int8 sf, mi;
    /* Key Signature (sf mi)
     * sf = -7:  7 flats
    ...
    ctl->cmsg(CMSG_INFO, VERB_NOISY, "Key signature: %d %s %s", abs(sf), (sf < 0) ? "flat(s)" : "sharp(s)", (mi) ? "minor" : "major");
}
...
/* Read a SMF track */
static int read_smf_track(struct midi_file *tf, int trackno, int rewindp)
{
        ...
        else if (me == 0xFF) /* Meta event */
        {
            ...
            if (type > 0 && type < 16)
            {
                                    ...
                                    current_read_track == 0))))
                {
                    ...
                    if ((text = readmidi_make_string_event(1, str, &ev, 1)) == NULL)
                    {
                        reuse_mblock(&tmpbuffer);
                        continue;
                    }
                            ...
                            *text = ME_KARAOKE_LYRIC;
                            if (karaoke_title_flag == 0 &&
                                ...
                                current_file_info->karaoke_title =
                                    add_karaoke_title(current_file_info->karaoke_title, str + 2);
            ...
            else
            {
                ...
                case 0x00:
                    if (len == 2)
                    {
                        a = tf_getc(tf);
                        b = tf_getc(tf);
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sequence Number %02x %02x)", a, b);
                    }
                    else
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sequence Number len=%d)", len);
                    break;
                ...
                case 0x54:
                     ...
                     */
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(SMPTE Offset meta event)");
                    skip(tf, len);
                    break;
                ...
                case 0x7f: /* Sequencer-Specific Meta-Event */
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sequencer-Specific meta event, length %ld)", len);
                    skip(tf, len);
                    break;
                ...
                case 0x20: /* MIDI channel prefix (SMF v1.0) */
                    if (len == 1)
                    {
                        ...
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(MIDI channel prefix %d)", midi_channel_prefix);
                    }
                    else
                        skip(tf, len);
                    break;
                ...
                case 0x21: /* MIDI port number */
                    if (len == 1)
                    {
                        if ((midi_port_number = tf_getc(tf)) == EOF)
                        {
                            ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Warning: %s: Too shorten midi file.", current_filename);
                            return -1;
                        }
                        midi_port_number &= 0xF;
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(MIDI port number %d)", midi_port_number);
                    }
                    else
                        skip(tf, len);
                    break;
                ...
                default:
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Meta event type 0x%02x, length %ld)", type, len);
                    skip(tf, len);
                    break;
                }
            }
        ...
        else /* MIDI event */
        {
            ...
            default: /* case 7: */
                ...
                case 2: /* Sys Com Song Position Pntr */
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sys Com Song Position Pntr)");
                    tf_getc(tf);
                    tf_getc(tf);
                    break;

                case 3: /* Sys Com Song Select(Song #) */
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sys Com Song Select(Song #))");
                    tf_getc(tf);
                    break;

                case 6: /* Sys Com tune request */
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sys Com tune request)");
                    break;
                case 8: /* Sys real time timing clock */
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sys real time timing clock)");
                    break;
                case 10: /* Sys real time start */
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sys real time start)");
                    break;
                case 11: /* Sys real time continue */
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sys real time continue)");
                    break;
                case 12: /* Sys real time stop */
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sys real time stop)");
                    break;
                case 14: /* Sys real time active sensing */
                    ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(Sys real time active sensing)");
                    break;
#if 0
		  case 15: /* Meta */
		  case 0: /* SysEx */
...
#endif
                default: /* 1, 4, 5, 9, 13 */
                    ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "*** Can't happen: status 0x%02X channel 0x%02X", laststatus, lastchan & 0xF);
                    break;
                }
            }
        }
                ...
                { /* -1 */
                    if (ch >= MAX_CHANNELS)
                    {
                        ...
                        while (newch < ch && newch < MAX_CHANNELS)
                        {
                            if (chidx[newch] == -1)
                            {
                                ctl->cmsg(CMSG_INFO, VERB_VERBOSE, "channel %d => %d", ch, newch);
                    ...
                    if (chidx[ch] == -1)
                    {
                        ...
                        else
                        {
                            ...
                            ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "channel %d => %d (mixed)", ch, newch);
                            ch = e->event.channel = chidx[ch] = newch;
                        }
                    }
                }
    ...
    for (j = 0; j < MAX_CHANNELS; j++)
    {
        ...
        else
        {
            if (tonebank[newbank] == NULL)
            {
                ...
                {
                    ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Tone bank %d is undefined", newbank);
    ...
    for (i = 0; i < event_count; i++)
    {
        ...
        if (!gch && IS_SET_CHANNELMASK(quietchannels, ch))
            ...
            case ME_RESET:
                ...
                ctl->cmsg(CMSG_INFO, VERB_NOISY, "MIDI reset at %d sec", (int)((double)st / play_mode->rate + 0.5));
                for (j = 0; j < MAX_CHANNELS; j++)
                {
                    if (play_system_mode == XG_SYSTEM_MODE && j % 16 == 9)
                        mapID[j] = XG_DRUM_MAP;
            ...
            case ME_PROGRAM:
                ...
                case GS_SYSTEM_MODE: /* GS */
                    ...
                    case 0: /* No change */
                        break;
                    case 1:
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(GS ch=%d SC-55 MAP)", ch);
                        mapID[ch] = (ISDRUMCHANNEL(ch)) ? SC_55_DRUM_MAP
                                                        : SC_55_TONE_MAP;
                        break;
                    case 2:
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(GS ch=%d SC-88 MAP)", ch);
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(GS ch=%d SC-8820/SC-8850 MAP)", ch);
                        mapID[ch] = (ISDRUMCHANNEL(ch)) ? SC_8850_DRUM_MAP
                                                        : SC_8850_TONE_MAP;
                        break;
                    default:
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(GS: ch=%d Strange bank LSB %d)", ch, bank_lsb[ch]);
                        break;
                    }
                    newbank = bank_msb[ch];
                    break;
                case XG_SYSTEM_MODE: /* XG */
                    ...
                    case 0: /* Normal */
                        if (ch == 9 && bank_lsb[ch] == 127 && mapID[ch] == XG_DRUM_MAP)
                            ...
                            ctl->cmsg(CMSG_WARNING, VERB_NORMAL, "Warning: XG bank 0/127 is found. It may be not correctly played.");
                        else
                        {
                            ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(XG ch=%d Normal voice)", ch);
                            midi_drumpart_change(ch, 0);
                            mapID[ch] = XG_NORMAL_MAP;
                        }
                        break;
                    case 64: /* SFX voice */
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(XG ch=%d SFX voice)", ch);
                        midi_drumpart_change(ch, 0);
                    ...
                    case 126: /* SFX kit */
                        ...
                        break;
                    case 127: /* Drum kit */
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(XG ch=%d Drum kit)", ch);
                        midi_drumpart_change(ch, 1);
                        mapID[ch] = XG_DRUM_MAP;
                        break;
                    default:
                        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "(XG: ch=%d Strange bank MSB %d)", ch, bank_msb[ch]);
                        break;
                    }
                    newbank = bank_lsb[ch];
                    break;
            ...
            case ME_NOTEON:
                ...
                if (ISDRUMCHANNEL(ch))
                {
                    ...
                    if (!drumset[newbank]) /* Is this a defined drumset? */
                    {
                        ...
                        {
                            ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Drum set %d is undefined", newbank);
        ...
        if ((dt = meep->event.time - at) && !counting_time)
        {
            ...
            if (st < 0)
            {
                ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Overflow the sample counter");
                free(groomed_list);
                return NULL;
            }
        }
...
static int read_smf_file(struct midi_file *tf)
{
    int32 len, divisions;
    ...
    if (tf_read(&len, 4, 1, tf) != 1)
    {
        if (errno)
            ...
            ctl->cmsg(CMSG_WARNING, VERB_NORMAL, "%s: Not a MIDI file!", current_filename);
        return 1;
    }
    len = BE_LONG(len);
    ...
    if (len > 6)
    {
        ctl->cmsg(CMSG_WARNING, VERB_NORMAL, "%s: MIDI file header size %ld bytes", current_filename, len);
...
void readmidi_read_init(void)
{
    ...
    readmidi_wrd_mode = 0; // WRD_TRACE_NOTHING;
    first = 0;
}
...
static void insert_note_steps(void)
{
    ...
    for (i = n = 0; i < event_count - 1 && n < 256 - 1; i++, e = e->next)
        if (e->event.type == ME_TIMESIG && e->event.channel == 0)
        {
            ...
            if (n > 0 && e->event.a == timesig[n - 1].a && e->event.b == timesig[n - 1].b)
                continue; /* unchanged */
...
static void insert_cuepoints(void)
{
    ...
    for (sp = time_segments; sp != NULL; sp = sp->next)
    {
        ...
        {
            if (sp->prev == NULL && (sp->begin.m.meas != 1 || sp->begin.m.beat != 1))
            {
...
static int32 compute_smf_at_time(const int32 sample, int32 *sample_adj)
{
    ...
    for (i = 0, e = evlist; i < event_count; i++, e = e->next)
    {
        st += (double)tempo * play_mode->rate / 1000000 / current_file_info->divisions * (e->event.time - prev_time) + 0.5;
...
static int32 compute_smf_at_time2(const Measure m, int32 *sample)
{
    ...
    {
        st += (double)tempo * play_mode->rate / 1000000 / current_file_info->divisions * (e->event.time - prev_time) + 0.5;
        if (e->event.type == ME_NOTE_STEP && ((e->event.a + ((e->event.b & 0x0f) << 8)) * 16 + (e->event.b >> 4)) >= m.meas * 16 + m.beat)
        {
            *sample = st;
            return e->event.time;
        }
...
MidiEvent *read_midi_file(struct midi_file *tf, int32 *count, int32 *sp,
                          char *fn)
{
    char magic[4];
    MidiEvent *ev;
...
struct midi_file *open_midi_file(char *fn, int decompress, int noise_mode)
{
    ...
    {
        tf = open_with_mem(infop->midi_data, infop->midi_data_size, noise_mode);
        if (infop->compressed)
        {
#ifdef DECOMPRESSOR
            if ((tf->url = url_inflate_open(tf->url, infop->midi_data_size, 1)) == NULL)
            {
                close_file(tf);
                return NULL;
            }
...
static void url_make_file_data(URL url, struct midi_file_info *infop)
{
    ...
    /* url => b */
    if ((compressor = open_deflate_handler(deflate_url_reader, url, ARC_DEFLATE_LEVEL)) == NULL)
        return;
...
static int check_need_cache(URL url, char *filename)
{
    ...
    t2 = url->type;
    return (t1 == URL_http_t || t1 == URL_ftp_t || t1 == URL_news_t) && t2 != URL_arc_t;
}
#else
/*ARGSUSED*/
static void url_make_file_data(URL url, struct midi_file_info *infop)
...
int midi_file_save_as(char *in_name, char *out_name)
{
    ...
    {
        ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "%s: %s", out_name, errno ? strerror(errno) : "Can't save file");
        return -1;
    }
    ...
    {
        size_t dummy = fwrite(buff, 1, n, ofp);
        ++dummy;
    }
...
{
    int i, j;
    for (i = 0; effect_parameter_xg[i].type_msb != -1 && effect_parameter_xg[i].type_lsb != -1; i++)
    {
        if (type_msb == effect_parameter_xg[i].type_msb && type_lsb == effect_parameter_xg[i].type_lsb)
        {
            for (j = 0; j < 16; j++)
            {
...
{
    int i, j;
    for (i = 0; effect_parameter_gs[i].type_msb != -1 && effect_parameter_gs[i].type_lsb != -1; i++)
    {
        if (msb == effect_parameter_gs[i].type_msb && lsb == effect_parameter_gs[i].type_lsb)
        {
            for (j = 0; j < 20; j++)
            {
...
void add_channel_layer(int to_ch, int from_ch)
{
    ...
    ctl->cmsg(CMSG_INFO, VERB_NOISY, "Channel Layer (CH:%d -> CH:%d)", from_ch, to_ch);
}
