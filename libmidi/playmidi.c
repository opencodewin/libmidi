...
#define SPEED_CHANGE_RATE 1.01 /*1.0594630943592953*/ /* 2^(1/12) */

/* Undefine if you don't want to use auto voice reduce implementation */
#ifdef AUTO_REDUCE_VOICE_ALLOW
#define REDUCE_VOICE_TIME_TUNING (play_mode->rate / 2) /* 0.5 sec */
#endif
...
int opt_default_module = MODULE_MIDI_DEFAULT;
...
static void reset_controllers(int c)
{
    ...
    if (prescanning_flag)
    {
        if (channel[c].volume > mainvolume_max)
        { /* pick maximum value of mainvolume */
...
static void redraw_controllers(int c)
{
    ...
    ctl_mode_event(CTLE_MUTE, 1, c, (IS_SET_CHANNELMASK(channel_mute, c)) ? 1 : 0);
...
void recompute_freq(int v)
{
    ...
    if (voice[v].vibrato_control_ratio || channel[ch].mod.val > 0)
    {
        ...
        if (opt_channel_pressure || opt_modulation_wheel)
        {
            ...
            vp->vibrato_depth += get_midi_controller_pitch_depth(&(channel[ch].mod)) + get_midi_controller_pitch_depth(&(channel[ch].bend)) + get_midi_controller_pitch_depth(&(channel[ch].caf)) + get_midi_controller_pitch_depth(&(channel[ch].paf)) + get_midi_controller_pitch_depth(&(channel[ch].cc1)) + get_midi_controller_pitch_depth(&(channel[ch].cc2));
            if (vp->vibrato_depth > VIBRATO_DEPTH_MAX)
            {
                vp->vibrato_depth = VIBRATO_DEPTH_MAX;
            }
            else if (vp->vibrato_depth < 1)
            {
                vp->vibrato_depth = 1;
            }
     ...
     * 1 fine = 2^5 tuning
     */
    tuning += (channel[ch].rpnmap[RPN_ADDR_0001] - 0x40 + (channel[ch].rpnmap[RPN_ADDR_0002] - 0x40) * 64) << 7;
    ...
    {
        tuning += (channel[ch].drums[note]->fine + channel[ch].drums[note]->coarse * 64) << 7;
    }
    /* MIDI controllers pitch control */
    if (opt_channel_pressure)
    {
        tuning += get_midi_controller_pitch(&(channel[ch].mod)) + get_midi_controller_pitch(&(channel[ch].bend)) + get_midi_controller_pitch(&(channel[ch].caf)) + get_midi_controller_pitch(&(channel[ch].paf)) + get_midi_controller_pitch(&(channel[ch].cc1)) + get_midi_controller_pitch(&(channel[ch].cc2));
    }
    if (opt_modulation_envelope)
    {
        if (voice[v].sample->tremolo_to_pitch)
        {
            tuning += lookup_triangular(voice[v].tremolo_phase >> RATE_SHIFT) * (voice[v].sample->tremolo_to_pitch << 13) / 100.0 + 0.5;
            channel[ch].pitchfactor = 0;
        }
        if (voice[v].sample->modenv_to_pitch)
        {
            tuning += voice[v].last_modenv_volume * (voice[v].sample->modenv_to_pitch << 13) / 100.0 + 0.5;
            channel[ch].pitchfactor = 0;
        }
    }
    /* GS/XG - Scale Tuning */
    ...
    if (!opt_pure_intonation && opt_temper_control && voice[v].temper_instant)
    {
        switch (tt)
        {
        case 0:
        ...
        case 2:
            if (current_temper_keysig < 8)
                f = freq_table_meantone[current_temper_freq_table + ((temper_adj) ? 36 : 0)][note];
            else
                f = freq_table_meantone[current_temper_freq_table + ((temper_adj) ? 24 : 12)][note];
            break;
        case 3:
            if (current_temper_keysig < 8)
                f = freq_table_pureint[current_temper_freq_table + ((temper_adj) ? 36 : 0)][note];
            else
                f = freq_table_pureint[current_temper_freq_table + ((temper_adj) ? 24 : 12)][note];
            break;
        default: /* user-defined temperament */
            if ((tt -= 0x40) >= 0 && tt < 4)
            {
                if (current_temper_keysig < 8)
                    f = freq_table_user[tt][current_temper_freq_table + ((temper_adj) ? 36 : 0)][note];
                else
                    f = freq_table_user[tt][current_temper_freq_table + ((temper_adj) ? 24 : 12)][note];
            }
            else
                f = freq_table[note];
            break;
    ...
    if (!voice[v].porta_control_ratio)
    {
        ...
        else
        {
            ...
            if (!channel[ch].pitchfactor)
            {
                ...
                if (tmp >= 0)
                    channel[ch].pitchfactor = bend_fine[tmp >> 5 & 0xff] * bend_coarse[tmp >> 13 & 0x7f];
                else
                    channel[ch].pitchfactor = 1.0 /
                                              (bend_fine[-tmp >> 5 & 0xff] * bend_coarse[-tmp >> 13 & 0x7f]);
            }
            voice[v].frequency =
    ...
    { /* Portamento */
        pb -= 0x2000;
        tmp = pb * channel[ch].rpnmap[RPN_ADDR_0000] + (voice[v].porta_pb << 5) + tuning;
        if (tmp >= 0)
            pf = bend_fine[tmp >> 5 & 0xff] * bend_coarse[tmp >> 13 & 0x7f];
        else
            pf = 1.0 / (bend_fine[-tmp >> 5 & 0xff] * bend_coarse[-tmp >> 13 & 0x7f]);
        voice[v].frequency = voice[v].orig_frequency * pf;
        voice[v].cache = NULL;
    }
    root_freq = voice[v].sample->root_freq;
    a = TIM_FSCALE(((double)voice[v].sample->sample_rate * ((double)voice[v].frequency + channel[ch].pitch_offset_fine)) / (root_freq * play_mode->rate), FRACTION_BITS) + 0.5;
...
static void recompute_voice_tremolo(int v)
{
    ...
    depth += get_midi_controller_amp_depth(&(channel[ch].mod)) + get_midi_controller_amp_depth(&(channel[ch].bend)) + get_midi_controller_amp_depth(&(channel[ch].caf)) + get_midi_controller_amp_depth(&(channel[ch].paf)) + get_midi_controller_amp_depth(&(channel[ch].cc1)) + get_midi_controller_amp_depth(&(channel[ch].cc2));
    if (depth > 256)
    {
        depth = 256;
    }
...
static void recompute_amp(int v)
{
     ...
     */
    if (!(play_mode->encoding & PE_MONO) && (opt_reverb_control || opt_chorus_control || opt_delay_control || (opt_eq_control && (eq_status_gs.low_gain != 0x40 || eq_status_gs.high_gain != 0x40)) || opt_insertion_effect))
        tempamp *= 1.35f * 0.55f;
    else
        tempamp *= 1.35f;
    ...
    if (opt_channel_pressure)
    {
        tempamp *= get_midi_controller_amp(&(channel[ch].mod)) * get_midi_controller_amp(&(channel[ch].bend)) * get_midi_controller_amp(&(channel[ch].caf)) * get_midi_controller_amp(&(channel[ch].paf)) * get_midi_controller_amp(&(channel[ch].cc1)) * get_midi_controller_amp(&(channel[ch].cc2));
...
void init_voice_filter(int i)
{
    ...
    if (opt_lpf_def && voice[i].sample->cutoff_freq)
    {
        ...
        if (voice[i].fc.orig_reso_dB < 0.0f)
        {
            voice[i].fc.orig_reso_dB = 0.0f;
        }
        if (opt_lpf_def == 2)
        {
            voice[i].fc.gain = 1.0;
...
void recompute_voice_filter(int v)
{
    ...
    if (opt_channel_pressure)
    {
        cent += get_midi_controller_filter_cutoff(&(channel[ch].mod)) + get_midi_controller_filter_cutoff(&(channel[ch].bend)) + get_midi_controller_filter_cutoff(&(channel[ch].caf)) + get_midi_controller_filter_cutoff(&(channel[ch].paf)) + get_midi_controller_filter_cutoff(&(channel[ch].cc1)) + get_midi_controller_filter_cutoff(&(channel[ch].cc2));
        depth_cent += get_midi_controller_filter_depth(&(channel[ch].mod)) + get_midi_controller_filter_depth(&(channel[ch].bend)) + get_midi_controller_filter_depth(&(channel[ch].caf)) + get_midi_controller_filter_depth(&(channel[ch].paf)) + get_midi_controller_filter_depth(&(channel[ch].cc1)) + get_midi_controller_filter_depth(&(channel[ch].cc2));
    }
    ...
    { /* Chamberlin filter */
        if (fc->freq > play_mode->rate / 6)
        {
            ...
            {
                fc->freq = play_mode->rate / 6;
            }
        }
        if (fc->reso_dB > CHAMBERLIN_RESONANCE_MAX)
        {
            fc->reso_dB = CHAMBERLIN_RESONANCE_MAX;
        }
    }
    else if (fc->type == 2)
    { /* Moog VCF */
...
float calc_drum_tva_level(int ch, int note, int level)
{
    ...
    if (def_level == -1 || def_level == 0)
    {
        def_level = 127;
    }
    else if (def_level > 127)
    {
        def_level = 127;
    }
...
static int32 calc_random_delay(int ch, int note)
{
    ...
    if (bank->tone[nprog].rnddelay == 0)
    {
        return 0;
    }
    else
    {
        return (int32)((double)bank->tone[nprog].rnddelay * play_mode->rate / 1000.0 * (get_pink_noise_light(&global_pink_noise_light) + 1.0f) * 0.5);
    }
}
...
Instrument *play_midi_load_instrument(int dr, int bk, int prog)
{
    ...
    if (dr && !tone->name && ((ip = tone->instrument) == MAGIC_LOAD_INSTRUMENT || ip == NULL) && (ip = load_instrument(dr, bk, prog)) != NULL)
    {
        tone->instrument = ip;
    ...
    else
    {
        ...
        if ((ip = tone0->instrument) == NULL || ip == MAGIC_LOAD_INSTRUMENT)
    ...
    if (load_success)
        aq_add(NULL, 0, 0); /* Update software buffer */
...
static int find_samples(MidiEvent *e, int *vlist)
{
    ...
    if (channel[ch].special_sample > 0)
    {
        ...
        {
            ctl->cmsg(CMSG_WARNING, VERB_VERBOSE, "Strange: Special patch %d is not installed", channel[ch].special_sample);
            return 0;
        }
        note = e->a + channel[ch].key_shift + note_key_offset;
        note = (note < 0) ? 0 : ((note > 127) ? 127 : note);
        return select_play_sample(s->sample, s->samples, &note, vlist, e, (s->type == INST_GUS) ? 1 : 32);
    }
    ...
    if (!prescanning_flag)
    {
        ...
        {
            j = vlist[i];
            if (!opt_realtime_playing && allocate_cache_size > 0 && !channel[ch].portamento)
            {
                              ...
                              int maxnv)
{
    ...
    else
    {
        if (opt_pure_intonation)
        {
            ...
            f = freq_table[*note];
        if (!opt_pure_intonation && opt_temper_control && tt == 0 && f != freq_table[*note])
        {
    ...
    for (i = 0, sp = splist; i < nsp; i++, sp++)
    {
        ...
        if (sp->low_freq <= fst && sp->high_freq >= fst && sp->low_vel <= vel && sp->high_vel >= vel && !(sp->inst_type == INST_SF2 && sp->sample_type == SF_SAMPLETYPE_RIGHT))
        {
    ...
    if (nv == 0)
    { /* we must select at least one sample. */
        fr = fc = 0;
        spc = spr = NULL;
        cdiff = 0x7fffffff;
        for (i = 0, sp = splist; i < nsp; i++, sp++)
    ...
    for (i = 0; i < nvc; i++)
    {
        ...
        if (spc->inst_type == INST_SF2 && spc->sample_type == SF_SAMPLETYPE_LEFT)
        {
            sample_link = spc->sf_sample_link;
            for (j = 0, sp = splist; j < nsp; j++, sp++)
                if (sp->inst_type == INST_SF2 && sp->sample_type == SF_SAMPLETYPE_RIGHT && sp->sf_sample_index == sample_link)
                {
    ...
    for (i = 0; i < upper_voices; i++)
        if (voice[i].status != VOICE_FREE && voice[i].channel == ch)
        {
            if (voice[i].note == note && (voice[i].status & status_check))
                ...
                kill_note(i);
            else if (voice[i].note == note && (channel[ch].assign_mode == 0 || (channel[ch].assign_mode == 1 &&
...
static void init_voice_vibrato(int v)
{
    ...
    nrpn_vib_flag = opt_nrpn_vibrato && (channel[ch].vibrato_ratio != 1.0 || channel[ch].vibrato_depth != 0);
...
static void init_voice_pan_delay(int v)
{
    ...
    if (opt_pan_delay && channel[ch].insertion_effect == 0 && !opt_surround_chorus)
    {
        if (vp->panning == 64)
        {
            vp->delay += pan_delay_table[64] * play_mode->rate / 1000;
        }
        else
        {
        ...
        if (vp->pan_delay_rpt < 1)
        {
            vp->pan_delay_rpt = 0;
        }
        vp->pan_delay_wpt = 0;
        vp->pan_delay_spt = vp->pan_delay_wpt - vp->pan_delay_rpt;
        if (vp->pan_delay_spt < 0)
        {
            vp->pan_delay_spt += PAN_DELAY_BUF_MAX;
        }
...
static void new_delay_voice(int v1, int level)
{
    ...
    vol = voice[v1].velocity * level / 127.0 * delay_status_gs.level_ratio[0];
...
static void new_chorus_voice_alternate(int v1, int level)
{
    ...
    if (note_adjusted > 127)
        note_adjusted = 127;
    else if (note_adjusted < 0)
        note_adjusted = 0;
...
static void note_on_prescan(MidiEvent *ev)
{
    ...
    if (ISDRUMCHANNEL(ch) &&
        channel[ch].drums[note] != NULL &&
        !get_rx_drum(channel[ch].drums[note], RX_NOTE_ON))
    { /* Rx. Note On */
        return;
    }
...
static void update_sostenuto_controls(int ch)
{
    ...
    if (ISDRUMCHANNEL(ch) || channel[ch].sostenuto == 0)
    {
        return;
    }
    ...
    for (i = 0; i < uv; i++)
    {
        if ((voice[i].status & (VOICE_ON | VOICE_OFF)) && voice[i].channel == ch)
        {
...
static void update_redamper_controls(int ch)
{
    ...
    if (ISDRUMCHANNEL(ch) || channel[ch].damper_mode == 0)
    {
        return;
    }
            ...
            (voice[i].status & (VOICE_ON | VOICE_SUSTAINED)))
        {
...
#endif
            {
                ...
                if (opt_surround_chorus) /* Surround chorus mode by Eric. */
                {
                    ...
                    if (!pan)
                        pan = 1; /* make hard left be 1 instead of 0 */
                    panlevel = 63;
                    if (pan - panlevel < 1)
                        panlevel = pan - 1;
                    if (pan + panlevel > 127)
...
void midi_program_change(int ch, int prog)
{
    ...
    case XG_SYSTEM_MODE: /* XG */
        ...
        case 0: /* Normal */
             ...
             * hardware / softsynth "bug" that is not reproduced by libmidi at the
             * moment.
             *
    ...
    case GM2_SYSTEM_MODE: /* GM2 */
        if ((channel[ch].bank_msb & 0xfe) == 0x78)
        { /* 0x78/0x79 */
...
static void process_sysex_event(int ev, int ch, int val, int b)
{
    ...
    { /* Universal system exclusive message */
        ...
        case 0x46: /* Rx. Note Off */
            ...
            ctl->cmsg(CMSG_INFO, VERB_NOISY, "Drum Instrument Rx. Note Off (CH:%d NOTE:%d VAL:%d)", ch, note, val);
            break;
        case 0x47: /* Rx. Note On */
            if (channel[ch].drums[note] == NULL)
                play_midi_setup_drums(ch, note);
            set_rx_drum(channel[ch].drums[note], RX_NOTE_ON, val);
            ctl->cmsg(CMSG_INFO, VERB_NOISY, "Drum Instrument Rx. Note On (CH:%d NOTE:%d VAL:%d)", ch, note, val);
            break;
    ...
    { /* GS system exclusive message */
        ...
        case 0x23: /* Insertion Effect ON/OFF */
            ...
            if (channel[ch].insertion_effect != val)
            {
                if (val)
                {
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "EFX ON (CH:%d)", ch);
                }
                else
                {
                    ctl->cmsg(CMSG_INFO, VERB_NOISY, "EFX OFF (CH:%d)", ch);
                }
            }
    ...
    { /* XG system exclusive message */
        ...
        {             /* Effect 2 */
            ...
            case 0x0A:
            case 0x0B:
                if (insertion_effect_xg[note].use_msb)
                {
                    break;
                }
                temp = b - 0x02;
        ...
        {             /* Effect 1 */
            ...
            case 0x40: /* Variation Type MSB */
                if (note >= XG_VARIATION_EFFECT_NUM || note < 0)
                {
                    break;
                }
        ...
        { /* Multi EQ */
            ...
            case 0x00: /* EQ type */
                if (opt_eq_control)
                {
                    if (val == 0)
                    {
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ type (0: Flat)");
                    }
                    else if (val == 1)
                    {
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ type (1: Jazz)");
                    }
                    else if (val == 2)
                    {
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ type (2: Pops)");
                    }
                    else if (val == 3)
                    {
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ type (3: Rock)");
                    }
                    else if (val == 4)
                    {
                        ctl->cmsg(CMSG_INFO, VERB_NOISY, "EQ type (4: Concert)");
                    }
...
static void update_rpn_map(int ch, int addr, int update_now)
{
    ...
    case NRPN_ADDR_0163: /* Attack Time */
        if (opt_tva_attack)
        {
            set_envelope_time(ch, val, EG_ATTACK);
        }
        break;
    case NRPN_ADDR_0164: /* EG Decay Time */
        if (opt_tva_decay)
        {
            set_envelope_time(ch, val, EG_DECAY);
        }
        break;
    case NRPN_ADDR_0166: /* EG Release Time */
        if (opt_tva_release)
        {
            set_envelope_time(ch, val, EG_RELEASE);
        }
        break;
    ...
    case RPN_ADDR_0003: /* Tuning Program Select */
        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Tuning Program Select (CH:%d VALUE:%d)", ch, val);
        for (i = 0; i < upper_voices; i++)
            if (voice[i].status != VOICE_FREE)
            {
    ...
    case RPN_ADDR_0004: /* Tuning Bank Select */
        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "Tuning Bank Select (CH:%d VALUE:%d)", ch, val);
        for (i = 0; i < upper_voices; i++)
            if (voice[i].status != VOICE_FREE)
            {
                voice[i].temper_instant = 1;
    ...
    case RPN_ADDR_0005: /* GM2: Modulation Depth Range */
        ...
        ctl->cmsg(CMSG_INFO, VERB_NOISY, "Modulation Depth Range (CH:%d VALUE:%d)", ch, channel[ch].rpnmap[RPN_ADDR_0005]);
        break;
        ...
        for (j = 0; j < MAX_CHANNELS; j += 16)
        {
            ...
            for (k = offset; k < offset + 16; k++)
            {
                ...
                case ME_SUSTAIN:
                    ...
                    if (channel[ch].damper_mode == 0)
                    { /* half-damper is not allowed. */
                        if (channel[ch].sustain >= 64)
                        {
...
static int playmidi_change_rate(int32 rate, int restart)
{
    ...
    {
        ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Out of sample rate: %d", rate);
        return -1;
    }
    ...
    {
        ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "Can't change sample rate to %d", rate);
        return -1;
    }
...
int check_apply_control(void)
{
    ...
    {
    case RC_SET_VOLUME:
        if (val > 0 && val <= MAX_AMPLIFICATION)
            amplification = val;
        adjust_amplification();
        ctl_mode_event(CTLE_MASTER_VOLUME, 0, amplification, 0);
        break;
...
static int apply_controls(void)
{
    ...
    do
    {
        ...
        case RC_TOGGLE_MUTE:
            ...
            sync_restart(0);
            jump_flag = 1;
            ctl_mode_event(CTLE_MUTE, 0, val, (IS_SET_CHANNELMASK(channel_mute, val)) ? 1 : 0);
            continue;
...
inline static int is_insertion_effect_xg(int ch)
{
    ...
    {
        if (variation_effect_xg[i].connection == XG_CONN_INSERTION && variation_effect_xg[i].part == ch)
        {
            return 1;
        }
    }
...
static void do_compute_data_midi(int32 count)
{
    ...
    channel_reverb = (stereo && (opt_reverb_control == 1 || opt_reverb_control == 3 || (opt_reverb_control < 0 && opt_reverb_control & 0x80)));
    ...
    channel_effect = (stereo && (channel_reverb || channel_chorus || channel_delay || channel_eq || opt_insertion_effect));
    ...
    if (channel_effect)
    {
        ...
        { /* allocating buffer for channel effect */
        ...
        for (i = 0; i < MAX_CHANNELS; i++)
        {
            ...
            else if (channel[i].eq_gs || (get_reverb_level(i) != DEFAULT_REVERB_SEND_LEVEL && current_sample - channel[i].lasttime < rev_max_delay_out) || channel[i].chorus_level > 0 || channel[i].delay_level > 0 || channel[i].eq_xg.valid || channel[i].dry_level != 127 || (opt_drum_effect && ISDRUMCHANNEL(i)) || is_insertion_effect_xg(i))
            {
        ...
        if (buf_index)
        {
            memset(reverb_buffer, 0, buf_index);
        }
    }
    ...
    if (play_system_mode == XG_SYSTEM_MODE && channel_effect)
    { /* XG */
        if (opt_insertion_effect)
        { /* insertion effect */
            for (i = 0; i < XG_INSERTION_EFFECT_NUM; i++)
        ...
        { /* system effects */
            ...
            if (p != buffer_pointer)
            {
                ...
                else
                {
                    ...
                    if (channel_reverb && channel[i].reverb_level > 0 && current_sample - channel[i].lasttime < rev_max_delay_out)
                    {
        ...
        if (channel_delay)
        {
            do_variation_effect1_xg(buffer_pointer, cnt);
        }
        if (channel_chorus)
        {
            do_ch_chorus_xg(buffer_pointer, cnt);
        }
        if (channel_reverb)
        {
            do_ch_reverb(buffer_pointer, cnt);
        }
        if (multi_eq_xg.valid)
        {
            do_multi_eq_xg(buffer_pointer, cnt);
        }
    }
    else if (channel_effect)
    { /* GM & GS */
        if (opt_insertion_effect)
        { /* insertion effect */
        ...
        if (channel_chorus)
        {
            do_ch_chorus(buffer_pointer, cnt);
        }
        if (channel_delay)
        {
            do_ch_delay(buffer_pointer, cnt);
        }
        if (channel_reverb)
        {
            do_ch_reverb(buffer_pointer, cnt);
...
static void do_compute_data_midi(int32 count)
{
    ...
    channel_effect = (stereo && (opt_reverb_control || opt_chorus_control || opt_delay_control || opt_eq_control || opt_insertion_effect));
    uv = upper_voices;
    for (i = 0; i < uv; i++)
        if (voice[i].status != VOICE_FREE)
            channel[voice[i].channel].lasttime = current_sample + count;
    ...
    if (channel_reverb)
    {
        ...
        {
            vc[i] = 0;
            if (channel[i].reverb_id != -1 && current_sample - channel[i].lasttime < REVERB_MAX_DELAY_OUT)
            {
                if (reverb_buffer == NULL)
                    reverb_buffer = (char *)safe_malloc(MAX_CHANNELS * AUDIO_BUFFER_SIZE * 8);
...
static int compute_data(int32 count)
{
    ...
    if (!count)
    {
        if (buffered_count)
        {
            ...
            if (aq_add(common_buffer, buffered_count, count) == -1)
                return RC_ERROR;
        }
        buffer_pointer = common_buffer;
        buffered_count = 0;
    ...
    while ((count + buffered_count) >= audio_buffer_size)
    {
        ...
        ctl->cmsg(CMSG_INFO, VERB_DEBUG_SILLY, "output data (%d)", audio_buffer_size);
        ...
        if (!opt_realtime_playing && (play_mode->flag & PF_CAN_TRACE))
        {
            if (!aq_fill_buffer_flag &&
                100 * ((double)(aq_filled() + aq_soft_filled()) / aq_get_dev_queuesize()) < 99)
            ...
            aq_get_dev_queuesize() > 0)
        {
            ...
            {
                filled_limit = play_mode->rate * reduce_voice_threshold / 1000 + 1; /* +1 disable zero */
            }
            ...
            rate = (int)(((double)(aq_filled() + aq_soft_filled()) / aq_get_dev_queuesize()) * 100 + 0.5);
            ...
            if (!opt_realtime_playing)
            {
                ...
                if (auto_reduce_polyphony && rate < 85)
                {
                    ...
                    else if (nv == voices && (rate > old_rate && filled > last_filled))
                    {
                        ok_nv_total += nv + 1;
                        ok_nv_counts++;
                    }
                    /* reduce polyphony when loosing buffer */
                    else if (rate < 75 && (rate < old_rate && filled < last_filled))
                    {
            ...
            if ((!opt_realtime_playing && rate < rate_limit) || filled < filled_limit)
            {
        ...
        if (aq_add(common_buffer, audio_buffer_size, count) == -1)
            return RC_ERROR;
...
int play_event(MidiEvent *ev)
{
    ...
    if (ctl->verbosity >= VERB_DEBUG_SILLY)
        ctl->cmsg(CMSG_INFO, VERB_DEBUG_SILLY, "Midi Event %d: %s %d %d %d", cet, event_name(ev->type), ev->channel, ev->a, ev->b);
    ...
    for (k = 0; k < MAX_CHANNELS; k += 16)
    {
        ...
        for (l = offset; l < offset + 16; l++)
        {
            ...
            if (layered)
            {
                if (!IS_SET_CHANNELMASK(channel[l].channel_layer, port_ch) || channel[l].port_select != (orig_ch >> 4))
                    continue;
                ev->channel = l;
            }
#endif
            ...
            case ME_TEMPER_TYPE:
                ...
                if (temper_type_mute)
                {
                    if (temper_type_mute & (1 << (current_event->a - ((current_event->a >= 0x40) ? 0x3c : 0))))
                    {
...
static void set_user_temper_entry(int part, int a, int b)
{
    ...
    case 5:
        ...
        for (i = 0; i < 11; i++)
        {
            if (((fh & 0xf) << 7 | fl) & 1 << i)
            {
                rf[i] *= (double)aa / bb * pow((double)cc / dd, (double)ee / ff);
                if (ifmax < i + 1)
                    ifmax = i + 1;
            }
            if (((bh & 0xf) << 7 | bl) & 1 << i)
            {
                rb[i] *= (double)aa / bb * pow((double)cc / dd, (double)ee / ff);
                if (ibmax < i + 1)
                    ibmax = i + 1;
            }
        }
...
static void read_header_wav(struct midi_file *tf)
{
    char buff[44];
    tf_read(buff, 1, 44, tf);
}

static int read_header_aiff(struct midi_file *tf)
{
    char buff[5] = "    ";
    int i;
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        if (strcmp(buff, "SSND") == 0)
        {
            ...
            tf_read(&buff[0], 1, 4, tf);
            ctl->cmsg(CMSG_INFO, VERB_NOISY, "aiff header read OK.");
            return 0;
        }
    }
...
static int load_pcm_file_wav()
{
    ...
    else if (strlen(pcm_alternate_file) >= 5 && strncasecmp(pcm_alternate_file + strlen(pcm_alternate_file) - 4, ".wav", 4) == 0)
        filename = safe_strdup(pcm_alternate_file);
    else
        return -1;

    ctl->cmsg(CMSG_INFO, VERB_NOISY, "wav filename: %s", filename);
    ...
    {
        ctl->cmsg(CMSG_INFO, VERB_NOISY, "open failed.");
        free(filename);
...
static int load_pcm_file_aiff()
{
    ...
    else if (strlen(pcm_alternate_file) >= 6 && strncasecmp(pcm_alternate_file + strlen(pcm_alternate_file) - 5, ".aiff", 5) == 0)
        filename = safe_strdup(pcm_alternate_file);
    else
        return -1;

    ctl->cmsg(CMSG_INFO, VERB_NOISY, "aiff filename: %s", filename);
    current_file_info->pcm_tf = open_file(filename, 0, OF_SILENT);
    if (current_file_info->pcm_tf)
    {
        ctl->cmsg(CMSG_INFO, VERB_NOISY, "open successed.");
...
{
    if (load_pcm_file_wav() == 0)
        return; /*load OK*/
    if (load_pcm_file_aiff() == 0)
        return; /*load OK*/
}
                               ...
                               int32 *nsamples)
{
    ...
    *event = read_midi_file(tf, &nevents, nsamples, fn);
    ...
    ctl_mode_event(CTLE_LOADING_DONE, 0, 0, (ptr_size_t)(*event));

    return RC_NONE;
}

static void ctl_time_signature(int sig_n, int sig_d, int sig_c, int sig_b)
{
    CtlEvent ce;
    ce.type = CTLE_TIME_SIG;
    ce.v1 = sig_n;
    ce.v2 = sig_d;
    ce.v3 = sig_c;
    ce.v4 = sig_b;
...
int play_midi_file(char *fn)
{
    ...
    restore_voices(0);
#endif
    ctl_mode_event(CTLE_TRACKS, 0, current_file_info->tracks, 0);
...
play_reload: /* Come here to reload MIDI file */
    ...
    ctl_time_signature(current_file_info->time_sig_n, current_file_info->time_sig_d, current_file_info->time_sig_c, current_file_info->time_sig_b);
...
static void ctl_timestamp(void)
{
    ...
    float last_msecs = NAN;

    msecs = (float)current_sample * 1000.f / (midi_time_ratio * play_mode->rate);
    for (i = voices = 0; i < upper_voices; i++)
        if (voice[i].status != VOICE_FREE)
            voices++;
    if (msecs == last_msecs && voices == last_voices)
        return;
    ce.type = CTLE_CURRENT_TIME;
    ce.v1 = last_msecs = msecs;
...
{
    float msecs;
    msecs = (float)samples * 1000 / (midi_time_ratio * play_mode->rate);
    ctl_mode_event(CTLE_CURRENT_TIME, 0, msecs, 0);
    ctl_mode_event(CTLE_REFRESH, 0, 0, 0);
}
...
static void ctl_prog_event(int ch)
{
    ...
    ce.v4 = (bank | (channel[ch].bank_lsb << 8) | (channel[ch].bank_msb << 16));
    if (ctl->trace_playing)
        push_midi_trace_ce(ctl->event, &ce);
    else
        ctl->event(&ce);
...
char *channel_instrum_name(int ch)
{
    ...
    if (tonebank[bank]->tone[prog].name)
    {
        ...
        if (comm == NULL)
        {
            comm = tonebank[bank]->tone[prog].name;
        }
    }
    else
    {
        ...
        if (comm == NULL)
        {
            comm = tonebank[0]->tone[prog].name;
        }
    }
 ...
 */
void playmidi_stream_init(int restart)
{
    int i;
    note_key_offset = key_adjust;
    midi_time_ratio = tempo_adjust;
    ...
    {
        init_mblock(&playmidi_pool);
        current_file_info = get_midi_file_info("MIDI", 1);
        midi_streaming = 1;
    }
    else
        reuse_mblock(&playmidi_pool);
    ...
    current_file_info->seq_name = safe_strdup("MIDI server");
...
void recompute_part_eq_xg(struct part_eq_xg *p)
{
    ...
    if (p->bass_freq >= 4 && p->bass_freq <= 40 && p->bass != 0x40)
    {
        ...
        {
            p->basss.gain = 0.19 * (double)(p->bass - 0x40);
        }
    ...
    if (p->treble_freq >= 28 && p->treble_freq <= 58 && p->treble != 0x40)
    {
        ...
        {
            p->trebles.gain = 0.19 * (double)(p->treble - 0x40);
        }
