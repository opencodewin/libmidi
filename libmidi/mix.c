...
#define DELAYED_MIXATION(a)                   \
    *lp++ += pan_delay_buf[pan_delay_spt];    \
    if (++pan_delay_spt == PAN_DELAY_BUF_MAX) \
    {                                         \
        pan_delay_spt = 0;                    \
    }                                         \
    pan_delay_buf[pan_delay_wpt] = (a)*s;     \
    if (++pan_delay_wpt == PAN_DELAY_BUF_MAX) \
    {                                         \
        pan_delay_wpt = 0;                    \
    }
...
void mix_voice(int32 *buf, int v, int32 c)
{
    ...
    if (vp->status == VOICE_DIE)
    {
        ...
        if (do_voice_filter(v, sp, filter_buffer, c))
        {
            sp = filter_buffer;
        }
        if (c > 0)
...
static inline int do_voice_filter(int v, resample_t *sp, mix_t *lp, int32 count)
{
    ...
    { /* copy with applying Moog lowpass VCF. */
        ...
        for (i = 0; i < count; i++)
        {
            ...
            t1 = b1;
            b1 = imuldiv24(x + b0, p) - imuldiv24(b1, f);
            t2 = b2;
            b2 = imuldiv24(b1 + t1, p) - imuldiv24(b2, f);
            t1 = b3;
            b3 = imuldiv24(b2 + t2, p) - imuldiv24(b3, f);
...
static inline void recalc_voice_resonance(int v)
{
    ...
    if (fc->reso_dB != fc->last_reso_dB || fc->q == 0)
    {
        ...
        else if (fc->type == 2)
        {
            ...
            if (fc->reso_lin > MOOG_RESONANCE_MAX)
            {
                fc->reso_lin = MOOG_RESONANCE_MAX;
            }
            else if (fc->reso_lin < 0)
            {
                fc->reso_lin = 0;
            }
        }
...
static inline void ramp_out(mix_t *sp, int32 *lp, int v, int32 c)
{
    ...
    int32 pan_delay_spt = vp->pan_delay_spt;
#endif
    ...
    mix_t *sp, int32 *lp, int v, int count)
{
    ...
    while (count)
        if (cc < count)
        {
            ...
            if (vp->pan_delay_rpt == 0)
            {
                for (i = 0; (vp->left_mix_offset | vp->right_mix_offset) && i < cc; i++)
                {
                    s = *sp++;
                    MIXATION(left);
                    MIXATION(right);
...
int recompute_envelope(int v)
{
     ...
     */
    if (stage == EG_GUS_RELEASE1 && vp->sample->modes & MODES_ENVELOPE && vp->status & (VOICE_ON | VOICE_SUSTAINED))
    {
        ...
        if (min_sustain_time > 0 || channel[ch].loop_timeout > 0)
        {
            ...
            envelope_width = sustain_time * play_mode->rate / (1000.0f * (double)control_ratio);
...
static inline int next_stage(int v)
{
    ...
    if (vp->envelope_volume == offset || (stage > EG_GUS_SUSTAIN && vp->envelope_volume < offset))
    ...
    else
    {
        if (vp->sample->envelope_keyf[stage]) /* envelope key-follow */
            rate *= pow(2.0, (double)(voice[v].note - 60) * (double)vp->sample->envelope_keyf[stage] / 1200.0f);
        val = channel[ch].envelope_rate[eg_stage];
    }
    if (vp->sample->envelope_velf[stage]) /* envelope velocity-follow */
        rate *= pow(2.0, (double)(voice[v].velocity - vp->sample->envelope_velf_bpo) * (double)vp->sample->envelope_velf[stage] / 1200.0f);
    ...
    if (stage > EG_GUS_SUSTAIN)
    {
        ...
        rate *= (double)vp->envelope_volume / vp->sample->envelope_offset[EG_GUS_ATTACK];
    ...
    { /* decaying phase */
        ...
        if (stage < EG_SF_DECAY && rate > OFFSET_MAX)
        { /* instantaneous decay */
            vp->envelope_volume = offset;
            return recompute_envelope(v);
        }
        else if (rate > vp->envelope_volume - offset)
        { /* fastest decay */
    ...
    { /* attacking phase */
        ...
        else if (rate > offset - vp->envelope_volume)
        { /* fastest attack */
...
static inline void update_tremolo(int v)
{
    ...
    if (vp->sample->inst_type == INST_SF2)
    {
        vp->tremolo_volume = 1.0 + TIM_FSCALENEG(
                                       lookup_sine(vp->tremolo_phase >> RATE_SHIFT) * depth * TREMOLO_AMPLITUDE_TUNING, 17);
    }
    else
    {
        vp->tremolo_volume = 1.0 + TIM_FSCALENEG(
    ...
    if (vp->panned == PANNED_MYSTERY)
    {
        ...
        if (vp->sample->modes & MODES_ENVELOPE)
        {
            if (vp->envelope_stage > 3)
                vp->last_envelope_volume = v_table[imuldiv16(vp->envelope_volume, vp->inv_envelope_scale) >> 20] * vp->envelope_scale;
            else if (vp->envelope_stage > 1)
                vp->last_envelope_volume = v_table[vp->envelope_volume >> 20];
            else
                vp->last_envelope_volume = attack_vol_table[vp->envelope_volume >> 20];
...
static inline int update_modulation_envelope(int v)
{
    ...
    if ((vp->modenv_increment < 0) ^ (vp->modenv_volume > vp->modenv_target))
    {
...
static inline int modenv_next_stage(int v)
{
    ...
    if (vp->modenv_volume == offset || (stage > EG_GUS_SUSTAIN && vp->modenv_volume < offset))
        return recompute_modulation_envelope(v);
    else if (stage < EG_SF_DECAY && rate > OFFSET_MAX)
    { /* instantaneous attack */
    ...
    else
    {
        if (vp->sample->modenv_keyf[stage]) /* envelope key-follow */
            rate *= pow(2.0, (double)(voice[v].note - 60) * (double)vp->sample->modenv_keyf[stage] / 1200.0f);
        val = channel[ch].envelope_rate[eg_stage];
    }
    if (vp->sample->modenv_velf[stage])
        rate *= pow(2.0, (double)(voice[v].velocity - vp->sample->modenv_velf_bpo) * (double)vp->sample->modenv_velf[stage] / 1200.0f);

    /* just before release-phase, some modifications are necessary */
    if (stage > EG_GUS_SUSTAIN)
    {
        /* adjusting release-rate for consistent release-time */
        rate *= (double)vp->modenv_volume / vp->sample->modenv_offset[EG_GUS_ATTACK];
    }

    /* regularizing envelope */
    if (offset < vp->modenv_volume)
    { /* decaying phase */
        if (val != -1)
        {
            if (stage > EG_DECAY)
...
int recompute_modulation_envelope(int v)
{
    ...
    if (stage == EG_GUS_RELEASE1 && vp->sample->modes & MODES_ENVELOPE && vp->status & (VOICE_ON | VOICE_SUSTAINED))
    {
        ...
        if (min_sustain_time > 0 || channel[ch].loop_timeout > 0)
        {
            ...
            /* Calculate the width of the envelope */
            modenv_width = sustain_time * play_mode->rate / (1000.0f * (double)control_ratio);
            vp->modenv_increment = -1;
            vp->modenv_target = vp->modenv_volume - modenv_width;
            if (vp->modenv_target < 0)
            {
                vp->modenv_target = 0;
            }
        }
