...
static resample_t resample_cspline(sample_t *src, splen_t ofs, resample_rec_t *rec)
{
    ...
    else
    {
        ...
        v1 = (((6 * v1 + ((((5 * v0 - 11 * v1 + 7 * temp - v3) >> 2) *
                            ofsf >>
                            FRACTION_BITS) *
                            (ofsf - (2L << FRACTION_BITS)) >>
                            FRACTION_BITS)) *
                    ((1L << FRACTION_BITS) - ofsf)) +
                    v2) /
                    (6L << FRACTION_BITS);
        return ((v1 > sample_bounds_max) ? sample_bounds_max : ((v1 < sample_bounds_min) ? sample_bounds_min : v1));
    }
}
...
static resample_t resample_lagrange(sample_t *src, splen_t ofs, resample_rec_t *rec)
{
    ...
    else
    {
        ...
        v3 >>= FRACTION_BITS;
        v3 += v0;
        return ((v3 > sample_bounds_max) ? sample_bounds_max : ((v3 < sample_bounds_min) ? sample_bounds_min : v3));
    }
}
...
static resample_t resample_gauss(sample_t *src, splen_t ofs, resample_rec_t *rec)
{
    ...
    if (temp_n < gauss_n)
    {
        ...
        y += *sptr;
        return ((y > sample_bounds_max) ? sample_bounds_max : ((y < sample_bounds_min) ? sample_bounds_min : y));
    }
    else
    {
        float *gptr, *gend;
        ...
        if (gauss_n == DEFAULT_GAUSS_ORDER)
        {
             ...
             */
#if __AVX2__
            __m256 y_f = _mm256_setzero_ps();
            // 8 float
            __m128i src_16 = _mm_lddqu_si128((__m128i const*)sptr);
            __m256i src_32 = _mm256_cvtepi16_epi32(src_16);
            __m256 src_f1 = _mm256_cvtepi32_ps(src_32);
            __m256 src_f2 = _mm256_loadu_ps(gptr);
            __m256 dst_f = _mm256_mul_ps(src_f1, src_f2);
            y_f = _mm256_hadd_ps(dst_f, y_f);
            // 8 float
            sptr += 8;
            gptr += 8;
            src_16 = _mm_lddqu_si128((__m128i const*)sptr);
            src_32 = _mm256_cvtepi16_epi32(src_16);
            src_f1 = _mm256_cvtepi32_ps(src_32);
            src_f2 = _mm256_loadu_ps(gptr);
            dst_f = _mm256_mul_ps(src_f1, src_f2);
            y_f = _mm256_hadd_ps(dst_f, y_f);
            // 8 float
            sptr += 8;
            gptr += 8;
            ...
            dst_f = _mm256_mul_ps(src_f1, src_f2);
            y_f = _mm256_hadd_ps(dst_f, y_f);
            y += y_f[0] + y_f[1] + y_f[2] + y_f[3] + y_f[4] + y_f[5] + y_f[6] + y_f[7];
            sptr += 8;
            gptr += 8;
            y += *(sptr++) * *(gptr++);
            y += *sptr * *gptr;
    ...
    else
    {
            ...
            (diff = (ofs >> FRACTION_BITS) - newt_old_trunc_x) > 0)
        {
            ...
            if (n_new <= newt_max)
            {
                ...
                newt_grow += diff;
                for (v1 = (ofs >> FRACTION_BITS) + (n_new >> 1) + 1, v2 = n_new; v2 > n_old; --v1, --v2)
                {
        ...
        v2 = n_new;
        y = newt_divd[v2][v2];
        xd = (double)(ofs & FRACTION_MASK) / (1L << FRACTION_BITS) + (newt_n >> 1) + newt_grow;
        for (--v2; v2; --v2)
        {
            y *= xd - v2;
            y += newt_divd[v2][v2];
...
void pre_resample(Sample *sp)
{
    ...
    ctl->cmsg(CMSG_INFO, VERB_DEBUG, " * pre-resampling for note %d (%s%d)", sp->note_to_use, note_name[sp->note_to_use % 12], (sp->note_to_use & 0x7F) / 12);
