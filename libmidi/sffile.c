...
static int READCHUNK(SFChunk *vp, struct midi_file *tf)
{
...
static int READDW(uint32 *vp, struct midi_file *tf)
{
...
static int READW(uint16 *vp, struct midi_file *tf)
{
...
static int READSTR(char *str, struct midi_file *tf)
{
    int n;
...
static int process_list(int size, SFInfo *sf, struct midi_file *fd)
{
    ...
    default:
        ctl->cmsg(CMSG_WARNING, VERB_NORMAL, "%s: *** illegal id in level 1: %4.4s", current_filename, chunk.id);
...
static int process_info(int size, SFInfo *sf, struct midi_file *fd)
{
    ...
    while (size > 0)
    {
        ...
        case IFIL_ID:
            ...
            ctl->cmsg(CMSG_INFO, VERB_DEBUG, "  version %d, minor %d", sf->version, sf->minorversion);
            break;
        case INAM_ID:
            /* name of the font */
            ...
            ctl->cmsg(CMSG_INFO, VERB_DEBUG, "  name %s", sf->sf_name);
            break;
...
static void load_sample_names(int size, SFInfo *sf, struct midi_file *fd)
{
    ...
    if (sf->version > 1)
    {
        ctl->cmsg(CMSG_WARNING, VERB_NORMAL, "%s: *** version 2 has obsolete format??", current_filename);
        FSKIP(size, fd);
        return;
    }
    ...
    {
        ctl->cmsg(CMSG_WARNING, VERB_NORMAL, "%s: *** different # of samples ?? (%d : %d)\n", current_filename, sf->nsamples, nsamples);
        FSKIP(size, fd);
        return;
    }
...
static void load_inst_header(int size, SFInfo *sf, struct midi_file *fd)
{
    ...
    for (i = 0; i < sf->ninsts; i++)
    {
        ...
        ctl->cmsg(CMSG_INFO, VERB_DEBUG, "  InstHdr %d (%s) bagNdx=%d", i, sf->inst[i].hdr.name, sf->inst[i].hdr.bagNdx);
    }
}
