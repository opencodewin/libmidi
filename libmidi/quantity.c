...
static int32 convert_VIBRATO_SWEEP_MS(int32 value, int32 vib_control_ratio)
{
    if (value <= 0)
        return 0;
    return (TIM_FSCALE((double)vib_control_ratio * 1000, SWEEP_SHIFT) / (double)(play_mode->rate * value));
}
...
#define QUANTITY_TYPE_INT(type)    \
    case QUANTITY_UNIT_TYPE(type): \
        REGISTER_TYPE_INT("", type##_NUM)
#define QUANTITY_TYPE_FLOAT(type)  \
    case QUANTITY_UNIT_TYPE(type): \
        REGISTER_TYPE_FLOAT("", type##_NUM)
